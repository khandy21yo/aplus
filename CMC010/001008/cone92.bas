...
40	  IF FNX$="" THEN &
		  V%=FNX%(DEVNAM$+PRGNUM$+LEFT(PRGNAM$,3%)+"C92.BAC",10%,"") &
...
8015	  PRINT "WORKING.  PLEASE WAIT." &
	...
	\ PRINT USING "FICA RATE FOR 1992 (EMPLOYEE):   #.####",D7 &
	\ PRINT USING "FICA LIMIT FOR 1992            ####.##",D8 &
...
8810	  D1=0.0 &
	\ D2=0.0 &
	\ D6=2300.		! Pers. Exemption Federal &
	\ D9=2000.		! Pers. Exemption State &
	\ D7=.0765		! FICA percentage &
	\ D8=4245.75		! FICA limit &
	\ COL.BASE=1950. &
	\ ORE.BASE=90. &
	\ KY.DED=650. &
	\ KY.EXEMP=20. &

8815	  READ H(I%,V%) FOR I%=1% TO 4% FOR V%=1% TO 2% &
	\ READ P0(I%,V%) FOR I%=1% TO 4% FOR V%=1% TO 2% &
	\ READ F(I%,V%) FOR I%=0% TO 3% FOR V%=1% TO 2% &
