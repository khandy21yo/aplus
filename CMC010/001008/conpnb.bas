...
91	  DATA	" ONE"," TWO"," THREE"," FOUR"," FIVE"," SIX"," SEVEN", &
...
6030	  GOSUB 7000 &
	\ GOSUB 4000 &
	\ PRINT "  JOB #   REG   OVT(CD)  RATE  REG ERN OVT ERN "; &
		"ML/SUB    OTHER       TOTAL"
6035	  B(I%)=0. FOR I%=1% TO 10% &
	\ GOTO 6050 IF X%>9% &
	\ STOP IF FNG%(4%,C$) &
	\ FOR J%=1% TO X% &
		\ GOSUB 2400 &
...
6572	  PRINT &
	\ RETURN
6575	  PRINT  FOR K%=X% TO 10% &
	\ PRINT "    FED  FICA   STATE" &
...
7000	  !
7010	  A1=A(0%,10%) &
	\ A1=A1+D(I%) FOR I%=3% TO 11% &
	\ Y=A(0%,8%)-A1
7022	  PRINT  FOR K%=1% TO 3% &
	\ IF Y<0. THEN &
		  PRINT &
		\ PRINT &
		\ PRINT "     VOID  VOID  VOID  VOID  VOID"+ &
			"  VOID VOID VOID VOID" FOR K%=1% TO 8% &
		\ PRINT &
		\ PRINT &
		\ GOTO 7040
7023	  PRINT TAB(72%); &
...
7025	  Y=A(0%,8%)-A1 &
	\ GOSUB 9600 &
	\ PRINT "       ";Y$ &
	\ PRINT TAB(57%);D$;TAB(67%); &
	\ PRINT USING "$$##,###,###.##", Y
7035	  PRINT &
	\ PRINT &
	\ PRINT TAB(10%);M$(2%) &
	\ PRINT TAB(10%);M$(3%) &
	\ PRINT TAB(10%);CVT$$(M$(4%),24%);"   ";CVT$$(M$(5%),24%) &
	\ PRINT
7040	  PRINT  FOR K%=1% TO 3% &
	\ PRINT  IF C1$="S" &
	\ RETURN
7100	  FOR I%=1% TO 4%
7110		  IF D$(I%+6%)=U$ AND D(I%+8%)<>0. THEN &
			  PRINT USING "###.##", D(I%+8%); &
			\ RETURN
7120	  NEXT I% &
	\ PRINT "      "; &
...
7130	  A1=0. &
	...
	\ FOR I%=1% TO 4% &
		\ U$=D$(I%+5%) &
		\ GOTO 7140 IF U$="U" OR U$="C" OR U$="S" &
		\ A1=A1+D(I%+7%) &
		\ A1$=U$+""
7140	  NEXT I%
7150	  GOTO 7160 IF A1=0. &
	\ PRINT USING " ###.## \\", A1,A1$;
7160	  IF A(0%,10%)=0. THEN &
		  PRINT &
		\ PRINT &
		\ RETURN
7170	  PRINT &
