10	  ! &
	  ! Program name: arage1		Compiled with SCALE 0 on V09.3 &
...
80	  DROP.DEAD.DATE$="        " &
	...
	\ IF DROP.DEAD.DATE$<>"" THEN  &
		  IF DROP.DEAD.DATE$<MID(SYS(DATE.TIME$),7%,8%) THEN  &
			...
			\ PRINT #CH%, MESG$; &
			\ V$=SYS(CHR$(9%))
100	  DIM ARTOGL.CODE$(30%), ARTOGL.DESC$(30%), TOTAL(30%,5%) &
...
200	  IF FNO%(DEVICE.CH%,"DEVICE.DAT","/RO","")=0% THEN  &
		...
		\ ARTOGL.DEVICE$=CVT$$(MID(FNL$,7%,20%),-1%)+"" IF FNG%(DEVICE.CH%,"ARTOGL")=0% &
		\ V%=FNC%(DEVICE.CH%)
210	  TEMP$="ARREG.DAT" &
...
305	  V%=FNC%(APCCTL.CH%) &
	\ V%=FNO%(ARREG.CH%,ARREG.DEVICE$+TEMP$,"/RO","") &
	\ IF FNS% THEN  &
...
380	  V%=FNC%(AGEDAY.CH%) &
	...
	\ IF FNO%(ARTOGL.CH%,ARTOGL.DEVICE$+"ARTOGL.DAT","/RO","") OR FNG%(ARTOGL.CH%,"") THEN  &
		  GOTO 395
390	  FIELD #ARTOGL.CH%+1%, FNL% AS JUNK$,2% AS ARTOGL.CODE$,20% AS ARTOGL.DESC$ &
...
510	  TITLE$="Customer Aging Report" &
	...
	\ PAGE.TOP1$="Cust # Customer Name"+SPACE$(19%)+" Phone "+SPACE$(13%)+"Current"+INTRVL$(2%)+INTRVL$(3%)+INTRVL$(4%)+INTRVL$(5%)+"       Balance"+CHR$(10%)+CHR$(13%) &
	\ USE1$="\    \ \"+SPACE$(23%)+"\     \\ \           \"+" #,###,###.## #,###,###.## #,###,###.##"+" #,###,###.## #,###,###.## ##,###,###.##" &
...
17030	  FIELD #CUSTOM.CH%+1%, FNL% AS JUNK$,6% AS CUSTOM.CODE$,25% AS CUSTOM.NAME$,25% AS CUSTOM.ADD1$,21% AS CUSTOM.ADD2$,15% AS CUSTOM.CITY$,2% AS CUSTOM.STATE$,5% AS CUSTOM.COUNTRY$,10% AS CUSTOM.ZIP$,10% AS CUSTOM.PHONE$,2% AS CUSTOM.SLSMCODE$,2% AS CUSTOM.SLSRCODE$,2% AS CUSTOM.LOC$,2% AS CUSTOM.CUSTYPE$,2% AS CUSTOM.BILTYPE$,1% AS CUSTOM.TAXFLG$,10% AS CUSTOM.STAXNUM$,2% AS CUSTOM.STAXLOC$,1% AS CUSTOM.METHOD$,1% AS CUSTOM.STMFLG$,15% AS CUSTOM.ALPSRT$,1% AS CUSTOM.SERVICE$ &
	\ GOTO 17395 IF FNCOMP%(CUSTOM.SLSMCODE$,SALMAN$)=0% AND SALMAN$<>"" &
	\ TEST.CUSNUM$=CUSTOM.CODE$+"" &
	\ TEST.CUSCOD$=CUSTOM.CUSTYPE$+"" &
	\ CUS.SLSMAN$=CUSTOM.SLSMCODE$+"" &
	\ CUS.NAM$=CUSTOM.NAME$+"" &
	\ PHONE$=CUSTOM.PHONE$+"" &
...
17410	  PRINT #OUTPUT.CH% &
	...
	\ GOSUB 17600 &
	\ MORE.LINES%=2%
17420	  FOR LOOP%=1% TO ARTOGL.LOOP% &
		\ FLAG%=0% &
		\ FLAG%=-1% IF TOTAL(LOOP%,I%)<>0. FOR I%=1% TO 5% &
		\ IF FLAG% THEN  &
			  PRINT #OUTPUT.CH%, USING USE1$, ARTOGL.CODE$(LOOP%),ARTOGL.DESC$(LOOP%),"","",TOTAL(LOOP%,1%),TOTAL(LOOP%,2%),TOTAL(LOOP%,3%),TOTAL(LOOP%,4%),TOTAL(LOOP%,5%),TOTAL(LOOP%,1%)+TOTAL(LOOP%,2%)+TOTAL(LOOP%,3%)+TOTAL(LOOP%,4%)+TOTAL(LOOP%,5%) &
...
18060	  PRINT #OUTPUT.CH%, USING USE1$, TEST.CUSNUM$,CUS.NAM$,CUS.SLSMAN$,FNFONE$(PHONE$),AGE(1%),AGE(2%),AGE(3%),AGE(4%),AGE(5%),BALANC &
	...
	\ GOTO 18070 IF TEST.CUSCOD$=ARTOGL.CODE$(LOOP%) FOR LOOP%=1% TO ARTOGL.LOOP% &
	\ IF TEST.CUSCOD$="" THEN &
		  LOOP%=0% &
	  ELSE &
		  ARTOGL.LOOP%,LOOP%=ARTOGL.LOOP%+1% &
		\ ARTOGL.CODE$(LOOP%)=TEST.CUSCOD$+"" &
		\ TOTAL(LOOP%,I%)=0. FOR I%=1% TO 5% &
		\ ARTOGL.DESC$(LOOP%)="** UNDEFINED **"
18070	  TOTAL(LOOP%,I%)=TOTAL(LOOP%,I%)+AGE(I%) FOR I%=1% TO 5%
