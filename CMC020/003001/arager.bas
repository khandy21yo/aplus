10	  ! &
	  ! Program name: arager		Compiled with SCALE 0 on V09.3 &
...
210	  ARREG.TEMP$=ARREG.DEVICE$+"ARREG.DAT" &
	...
	\ IF ORDER.KEY$="C" THEN &
	  ...
	  ELSE &
		  PAR.LEN%=15%
220	  IF CHAIN.FLAG% OR ORDER.KEY$="C" THEN  &
		  GOTO 300
...
260	  POINTER%=FNR(CUSTOM.CH%+1%) &
	\ FIELD #CUSTOM.CH%+1%, FNL% AS TEMP$,6% AS CUSTOM.CUSNUM$,138% AS TEMP$,15% AS CUSTOM.ALPHA$,1% AS CUSTOM.SERVICE$,6% AS CUSTOM.LOC$,4% AS CUSTOM.SEQ$,1% AS CUSTOM.PURGE$ &
	\ SORT.KEY$(RECORD.COUNT%)=CUSTOM.LOC$+CUSTOM.SEQ$+CUSTOM.CUSNUM$+CVT%$(POINTER%) &
	\ RECORD.COUNT%=RECORD.COUNT%+1% &
	\ IF FNN%(CUSTOM.CH%)=0% THEN  &
		  GOTO 240
280	  FOR LOOP%=RECORD.COUNT% TO END.RECORD% &
		\ SORT.KEY$(LOOP%)=SPACE$(16%)+CVT%$(-1%) &
	\ NEXT LOOP%
290	  CLOSE TEMP.CH% &
	\ Q0$=SYS(CHR$(7%)) &
	...
	\ Q3$=CVT%$(30999%)+PRJPRG$ &
	\ Q3$=Q3$+"  " &
	\ Q9$=CHR$(13%)+WORK.TEMP$+CHR$(13%)+CHR$(13%)+Q0$+CHR$(14%)+ORDER.KEY$ &
...
380	  CUSBAL.FLAG%=FNO%(CUSBAL.CH%,CUSBAL.DEVICE$+"CUSBAL.DAT","","") &
	\ IF CUSBAL.FLAG% AND CUSBAL.FLAG%<>5% THEN  &
		  V$=FNMESS$(CH%,FNS%,"CUSBAL.DAT",0%,-1%) &
		\ GOTO 17540
...
18060	  PRINT #OUTPUT.CH%, USING USE1$, TEST.CUSNUM$,CUS.NAM$,LOCATION$,FNFONE$(PHONE$),AGE(1%),AGE(2%),AGE(3%),AGE(4%),AGE(5%),BALANC &
