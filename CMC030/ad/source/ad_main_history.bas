1	%TITLE "Asset Depreciation History File"
	...
	!	.b
	!	.lm +5
	!	The ^*History\* file is updated each time the Depreciation
	!	Ledger file is updated.
	!	.b
	!	The information contained in this file is not meant to be edited.
	!	.b
	!	From the ^*History\* file the amount of depreciation for previous
	!	periods may be printed.
	!	.lm -5
	!
	! Index:
	...
	! Compile:
	!
	!	$ BAS AD_SOURCE:AD_MAIN_HISTORY/LINE
	...
	MAP (AD_HISTORY_OLD)	AD_HISTORY_CDD		AD_HISTORY_OLD, &
		AD_HISTORY2
	...
	CASE OPT_INIT
		...
		! Define window
		!
		SMG_WINDOW::DESCR = "Asset depreciation history"
		SMG_WINDOW::NHELP = "AD_MAIN_HISTORY"
		SMG_WINDOW::HSIZE = 78%
		SMG_WINDOW::VSIZE = 18%
		SMG_WINDOW::HPOS  = 2%
		SMG_WINDOW::VPOS  = 2%
		...
		AD_HISTORY::DEP_STATUS = "A"

		CALL READ_DEFAULTS(SMG_WINDOW)

20700		IF (AD_HISTORY.CH% <= 0%)
		THEN
			...
			USE
				CALL ENTR_3MESSAGE(SCOPE, &
					"Unable to open AD_HISTORY file " + NUM1$(ERR), 0%)
				CONTINUE 27000
			END WHEN
		END IF

20710		SMG_WINDOW::CHAN  = AD_HISTORY.CH%
...
20800	CASE OPT_ENTRY
 ...
 ReEnter:	SCOPE::SCOPE_EXIT = 0%
	...
	!	.b
	!	.lm +5
	!	The Asset _# field contains the number of a
	!	particular asset.
	!	.b
	!	An entry is required in this field.  Ten (10) spaces are
	!	provided for the entry.
	!	.b
	!	Pressing ^*<List Choices>\* while the cursor is located
	!	at this field will provide a list of valid Asset _#'s.
	!	.lm -5
	!
	! Index:
	!	.x Asset>Number
	...
	!	.b
	!	Pressing ^*<List Choices>\* while the cursor is located
	!	at this field will provide a list of the valid object codes.
	!	.lm -5
	!
	! Index:
	!	.x Object>Asset Depreciation History File
	!	.x Asset Depreication History File>Object
	!
	!--
	...
	!
	! Index:
	!	.x Period>Asset Depreication History File
	!	.x Asset Depreication History File>Period
	!
	!--
			...
			IF MAIN_WINDOW(AD_MAIN_CONTROLOBJ.ID, &
				"Q0" + AD_HISTORY::DEP_OBJECT) = 1%
			THEN
				ERA$ = AD_CONTROLOBJ::ERA
			END IF

			AD_HISTORY::PERIOD = ENTR_PERIOD(SMG_WINDOW::WNUMBER, &
				"06;23", TEMP$, &
				AD_HISTORY::PERIOD, MFLAG, ERA$, &
				MVALUE)
	...
	!	.b
	!	.lm +5
	!	The ^*Amount\* field contains the dollar amount
	!	which has been depreciated for this asset, with this
	!	particular Object code, for this particular period
	!	of time.
	!	.b
	...
	!--
			...
			AD_HISTORY::AMOUNT_HIS = &
				...
				"7;23", TEMP$, AD_HISTORY::AMOUNT_HIS, MFLAG, &
				"#,###,###.##", MVALUE)
	...
	!	.b
	!	.lm +5
	!	The ^*Units\* field contains the quantity of Units which
	!	has been depreciated for this Asset, with this particular
	!	Object code, in this particular period of time.
	!	.b
	!	The field may contain a figure as large as 9,999,999.99.
	!	.lm -5
	!
	! Index:
	!	.x Units>Asset Depreciation History File
	!	.x Asset Depreication History File>Units
	!
	!--
			...
			AD_HISTORY::UNIT_HIS = &
				ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
				"8;23", TEMP$, AD_HISTORY::UNIT_HIS, MFLAG, &
				"#,###,###.##", MVALUE)
...
20850	CASE OPT_TESTENTRY
		...
		CASE 1%
			...
			!
			AD_MAIN_HISTORY = FUNC_TESTENTRY(SMG_WINDOW, &
				AD_HISTORY::ASSET_NUM, &
	...
	!
	! Set AD_HISTORY_OLD value
	!
20900	CASE OPT_SETOLD
		AD_HISTORY_OLD = AD_HISTORY

	!
	! Restore AD_HISTORY_OLD value
	!
	CASE OPT_RESETOLD
		AD_HISTORY = AD_HISTORY_OLD

	!
	! Set default value
	!
	CASE OPT_SETDEFAULT
		AD_HISTORY2 = AD_HISTORY

	!
	! Restore default value
	!
	CASE OPT_RESETDEFAULT
		AD_HISTORY = AD_HISTORY2
	...
	CASE OPT_VIEW
		...
		!
		CASE 2%
			MVALUE = "013,020,027,038,049"

		!
		! Convert current record into text
		!
		CASE 3%
			MVALUE = AD_HISTORY::ASSET_NUM + " " + &
				AD_HISTORY::DEP_OBJECT + "      " + &
				AD_HISTORY::PERIOD + " " + &
				FORMAT$(AD_HISTORY::AMOUNT_HIS, &
					"#######.##") + " " + &
				FORMAT$(AD_HISTORY::UNIT_HIS, &
					"#######.##")
	...
	CASE OPT_FIND
		...
		CASE 2%
			FIND #SMG_WINDOW::CHAN, &
				KEY #2% GE AD_HISTORY::DEP_OBJECT + &
					AD_HISTORY::PERIOD + &
					AD_HISTORY::ASSET_NUM, &
				REGARDLESS
