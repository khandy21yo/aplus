1	%TITLE "Fixed Asset by Property Type"
	%SBTTL "AD_RPRT_FIXPROPTYPE"
	%IDENT "V3.6a Calico"
	...
	!	.b
	!	.lm +5
	!	The ^*Fixed Asset by Property Type\* prints
	!	a report containing the year to date depreciation not including the current
	!	period calculations. The report is printed in Property Type  order and may
	...
	!
	! Index:
	!	.x Report>Depreiciation Assets by Property Type
	!	.x Depreiciation Assets by Property Type>Report
	!
	! Option:
	!
	!
	! Compile:
	!
	!	$ BAS AD_SOURCE:AD_RPRT_FIXPROPTYPE/LINE
	!	$ LINK/EXE=AD_EXE: AD_RPRT_FIXPROPTYPE, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE AD_RPRT_FIXPROPTYPE.OBJ;*
	!
	! AUTHOR:
	!
	!	09/24/88 - Frank F. Starman
	!
	! MODIFICATION HISTORY:
	!
	!	02/26/91 - Craig Tanner
	!		Upgraded to use AD_35ASSET; no longer needs AD_RETIRED.
	!
	...
	!	causes the report to begin with the selected type.
	!	.b
	!	A blank field will cause the report to begin with the
	!	first property type in the file.
	!	.lm -5
	!
	! Index:
	!	.x From Property Type>Fixed Assets by Property Type
	!	.x Fixed Assets by Property Type>From Property Type
	!
	!--
	...
	!	.b
	!	.lm +5
	!	The ^*To Property Type\* field
	!	causes the report to end with the selected type.
	!	.b
	...
	!
	! Index:
	!	.x To Property Type>Fixed Assets>By Property Type
	!	.x Fixed Assets>By Property Type>To Property Type
	!
	!--
	...
	!	including selected property types using the "wildcarding"
	!	technique.
	!	.lm -5
	!
	! Index:
	!	.x Wildcard>Fixed Assets>By Property Type
	!	.x Fixed Assets>By Property Type>Wildcard
	!
	!--
	...
	!
	! Index:
	!	.x Object>Fixed Assets>By Property
	!	.x Fixed Assets>By Property>Object
	!
	!--
	...
	!	entering ^*N\* or print only report totals by entering ^*Y\*.
	!	.lm -5
	!
	! Index:
	!	.x Only Totals>Fixed Assets>By Property Type
	!	.x Fixed Assets>By Property Type>Only Totals
	!
	!--
	...
	!	.b
	!	.lm +5
	!	The ^*Date\* field causes the report
	!	to begin with the selected date.
	!	.b
	!	A blank field will cause the report to being with the
	...
	!
	! Index:
	!	.x From Date>Fixed Assets>By Property
	!	.x Fixed Assets>By Property>From Date
	!
	!--
	...
	!	.b
	!	A blank field causes the report to print to the end
	!	of the file.
	!	.b
	!	The format for entry is MMDDYYYY or MMDDYY.
	!	.lm -5
	!
	! Index:
	!	.x To Date>Fixed Assets>By Property Type
	!	.x Fixed Assets>By Property Type>To Date
	!
	!--

	TIME.PERIOD$ = ""
	TIME.PERIOD$ = "  FROM " + PRNT_DATE(FROM_DATE$, 8%) &
		IF FROM_DATE$ <> "" AND TO_DATE$ = ""
	TIME.PERIOD$ = "  TO " + PRNT_DATE(TO_DATE$, 8%) &
		IF FROM_DATE$ = "" AND TO_DATE$ <> ""
	TIME.PERIOD$ = "  FROM " + PRNT_DATE(FROM_DATE$, 8%) + " TO " + &
		PRNT_DATE(TO_DATE$, 8%) &
		IF FROM_DATE$ <> "" AND TO_DATE$ <> ""
...
420	IF AD_DEPRECIATION::DEPCLASS <> AD_DEPCLASS::DEPCLASS
	THEN
		WHEN ERROR IN
			GET #AD_DEPCLASS.CH%, &
				...
				REGARDLESS
		USE
			AD_DEPCLASS::DEPCLASS = AD_DEPRECIATION::DEPCLASS
...
430	PUT #AD_TEMP.CH%
 ...
 ReportTitle:
	TITLE$(1%) = "DEPRECIATION  ASSET  BY  PROPERTY  TYPE"
	TITLE$(2%) = "FOR  OBJECT  " + OBJECT$ + " " + &
		TRM$(AD_OBJECT::DESCRIPTION) + TIME.PERIOD$
...
17400	!
	...
	THEN
		...
		TEXT$ = "Grand Total" + &
			...
			FORMAT$(A_ITCREDUCE + F_ITCREDUCE + N_ITCREDUCE + R_ITCREDUCE, &
				"######.##") + &
			SPACE$(34%) + &
			FORMAT$(A_ADJUST_BASIS + F_ADJUST_BASIS + N_ADJUST_BASIS + &
				R_ADJUST_BASIS, "##,###,###.##") + "   " + &
