1	%TITLE "Accounts Payable Close File Maintenance"
	%SBTTL "AP_MAIN_CLOSE_MAINT"
	%IDENT "V3.6a Calico"

	FUNCTION LONG AP_MAIN_CLOSE_MAINT(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	!	.b
	!	.lm +5
	!	This option maintains the ^*Close\* file for the marketing
	!	system.
	!	.lm -5
	!
	! Index:
	...
	! Compile:
	!
	!	$ BAS AP_SOURCE:AP_MAIN_CLOSE_MAINT/LINE
	...
	!		Cleaned code relating to FUNC_TESTENTRY.
	!
	!	02/02/93 - Kevin Handy
	!		Added batch number to maintenance screen.
	!
	!	02/02/93 - Kevin Handy
	!		Added invoice number and batch number as
	!		keys.
	!
	...
	!		Added close date to maintained fields
	!
	!	08/23/2000 - Kevin Handy
	!		Lose references to AP_OPEN, which isn't really
	!		wanted/needed in this code, and may cause bugs.
	!		Add error trapping in 'GetTrans'.
	!		Use WHEN ERROR IN.
	!--
	...
	MAP (AP_CLOSE_2)	AP_CLOSE_CDD	AP_CLOSE_OLD,	AP_CLOSE2
	...
	CASE OPT_INIT
		...
		SMG_WINDOW::NHELP = "AP_MAIN_CLOSE_MAINT"
		SMG_WINDOW::HSIZE = 78%
		SMG_WINDOW::VSIZE = 18%
		SMG_WINDOW::HVIEW = 78%
		SMG_WINDOW::VVIEW = 18%
...
700		!
		...
		!
750		%INCLUDE "SOURCE:[AP.OPEN]AP_CLOSE.CRE"
		AP_CLOSE.READONLY% = 0%

790		SMG_WINDOW::CHAN  = AP_CLOSE.CH%
		RESET #AP_CLOSE.CH%
...
20200	!******************************************************************
 ...
 Reenter2:	SCOPE::SCOPE_EXIT = 0%
	...
	!
	!--
			AP_CLOSE::VENNUM = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "02;24", TEMP$, &
				AP_CLOSE::VENNUM, MFLAG, "'E", MVALUE)

			SELECT SCOPE::SCOPE_EXIT
			CASE SMG$K_TRM_F14
				AP_CLOSE::VENNUM = AP_VENDOR::VENNUM &
	...
	!
	!--
			AP_CLOSE::TRANKEY = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "4;24", TEMP$, &
				AP_CLOSE::TRANKEY, MFLAG, "'E", MVALUE)
	...
	!
	!--
			AP_CLOSE::TRANKEY_DATE = ENTR_3DATE(SCOPE, &
				SMG_WINDOW::WNUMBER, "05;24", TEMP$, &
				AP_CLOSE::TRANKEY_DATE, MFLAG, "8", &
				MVALUE)
	...
	!
	!--
			AP_CLOSE::INVNUM = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "7;24", TEMP$, &
				AP_CLOSE::INVNUM, MFLAG, "'E", MVALUE)
	...
	!
	!--
			AP_CLOSE::INVDAT = ENTR_3DATE(SCOPE, &
				SMG_WINDOW::WNUMBER, "8;29", TEMP$, &
				AP_CLOSE::INVDAT, MFLAG, "8", MVALUE)
	...
	!
	!--
			AP_CLOSE::INVAMT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, "9;25", TEMP$, &
				AP_CLOSE::INVAMT * 1.0, MFLAG, &
				"###,###,###.##", MVALUE)
	...
	!
	!--
			AP_CLOSE::CODE_1099 = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "11;37", TEMP$, &
				AP_CLOSE::CODE_1099, MFLAG, "'E", &
				MVALUE)

			SELECT SCOPE::SCOPE_EXIT
			CASE SMG$K_TRM_F14
				AP_CLOSE::CODE_1099 = AP_1099_TABLE::CODE &
	...
	!
	!--
			AP_CLOSE::AMT_1099 = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, "12;25", TEMP$, &
				AP_CLOSE::AMT_1099 * 1.0, MFLAG, &
				"###,###,###.##", MVALUE)
	...
	!
	!--
			AP_CLOSE::USE_JOB_NUM = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "13;29", TEMP$, &
				AP_CLOSE::USE_JOB_NUM, MFLAG, "'E", &
				MVALUE)
	...
	!
	!--
			AP_CLOSE::USE_AMT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, "14;25", TEMP$, &
				AP_CLOSE::USE_AMT * 1.0, MFLAG, &
				"###,###,###.##", MVALUE)
	...
	!
	!--
			AP_CLOSE::DISCDAT = ENTR_3DATE(SCOPE, &
				SMG_WINDOW::WNUMBER, "16;29", TEMP$, &
				AP_CLOSE::DISCDAT, MFLAG, "8", MVALUE)
	...
	!
	!--
			AP_CLOSE::DISAMT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, "17;25", TEMP$, &
				AP_CLOSE::DISAMT * 1.0, MFLAG, &
				"###,###,###.##", MVALUE)
	...
	!
	!--
			AP_CLOSE::DUEDAT = ENTR_3DATE(SCOPE, &
				SMG_WINDOW::WNUMBER, "4;59", TEMP$, &
				AP_CLOSE::DUEDAT, MFLAG, "8", MVALUE)
	...
	!
	!--
			AP_CLOSE::PONUM = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "5;59", TEMP$, &
				AP_CLOSE::PONUM, MFLAG OR 2%, "~R 'E", MVALUE)
	...
	!	.b
	!	.lm +5
	!	During initialization procedures, the ^*Accounts Payable Account\* field
	!	enters the
	...
	!
	!--
			AP_CLOSE::AP_ACCT = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "7;59", TEMP$, &
				AP_CLOSE::AP_ACCT, MFLAG, "'E", MVALUE)

			IF (SCOPE::SCOPE_EXIT = SMG$K_TRM_F14)
			THEN
				IF (MAIN_WINDOW(GL_MAIN_CHART.ID, "VX") = 1%)
				THEN
					AP_CLOSE::AP_ACCT = GL_CHART::ACCT
				END IF
				GOTO Reenter2
			END IF
	...
	!
	!--
			AP_CLOSE::CASH_ACCT = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "8;59", TEMP$, &
				AP_CLOSE::CASH_ACCT, MFLAG, "'E", &
				MVALUE)
			...
			THEN
				IF (MAIN_WINDOW(GL_MAIN_CHART.ID, "VX") = 1%)
				THEN
					AP_CLOSE::CASH_ACCT = GL_CHART::ACCT
				END IF
				GOTO Reenter2
			END IF
	...
	!
	!--
			AP_CLOSE::CKNUM = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "11;59", TEMP$, &
				AP_CLOSE::CKNUM, MFLAG, "'E", MVALUE)
	...
	!
	!--
			AP_CLOSE::CKDAT = ENTR_3DATE(SCOPE, &
				SMG_WINDOW::WNUMBER, "12;59", TEMP$, &
				AP_CLOSE::CKDAT, MFLAG, "8", MVALUE)
	...
	!
	!--
			AP_CLOSE::CKDESC = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "13;59", TEMP$, &
				AP_CLOSE::CKDESC, MFLAG, "'E", MVALUE)
	...
	!
	!--
			AP_CLOSE::CKAMT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, "14;59", TEMP$, &
				AP_CLOSE::CKAMT * 1.0, MFLAG, &
				"###,###,###.##", MVALUE)
	...
	!	.b
	!	.lm +5
	!	The ^*Post Period\* field indicates the period in which the
	...
	!
	!--
			AP_CLOSE::UPDATED = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "16;59", TEMP$, &
				AP_CLOSE::UPDATED, MFLAG, "8", &
				MVALUE)
	...
	!
	!--
			AP_CLOSE::BATCH = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "17;59", TEMP$, &
				AP_CLOSE::BATCH, MFLAG, "'E", MVALUE)
	...
	!
	!--
			AP_CLOSE::CLOSEDATE = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "18;59", TEMP$, &
				AP_CLOSE::CLOSEDATE, MFLAG, "'E", MVALUE)
...
21300	CASE OPT_TESTENTRY
		AP_MAIN_CLOSE_MAINT = 0%

		SELECT MLOOP
		CASE 1%
			AP_MAIN_CLOSE_MAINT = FUNC_TESTENTRY(SMG_WINDOW, &
				AP_CLOSE::VENNUM, AP_VENDOR::VENNAM, &
				"AP", MLOOP, "VENNUM", &
		...
		CASE 7%
			AP_MAIN_CLOSE_MAINT = FUNC_TESTENTRY(SMG_WINDOW, &
				AP_CLOSE::CODE_1099, AP_1099_TABLE::DESCR, &
				"AP", MLOOP, "1099CD", &
		...
		CASE 15%, 16%
			...
			!
			TEMP$ = AP_CLOSE::AP_ACCT
			TEMP$ = STRING$(LEN(TEMP$), 63%) IF TEMP$ = ""
			TEMP$ = AP_CLOSE::CASH_ACCT IF MLOOP = 16%
	...
	!
	! Set AP_CLOSE_OLD value
	!
21500	CASE OPT_SETOLD
		AP_CLOSE_OLD = AP_CLOSE

	!
	! Restore AP_CLOSE_OLD value
	!
	CASE OPT_RESETOLD
		AP_CLOSE = AP_CLOSE_OLD
		GOSUB GetTrans
	...
	CASE OPT_VIEW
		...
		!
		CASE 3%
			MVALUE = AP_CLOSE::VENNUM + " " + &
				AP_CLOSE::TRANKEY + " " + &
				AP_CLOSE::INVNUM + "  " + &
				PRNT_DATE(AP_CLOSE::INVDAT, 8%) + "  " + &
	...
	CASE OPT_FIND
		SELECT MLOOP
		CASE 0%
			FIND #AP_CLOSE.CH%, KEY #0% &
				GE AP_CLOSE::VENNUM + AP_CLOSE::TRANKEY, &
				REGARDLESS
		CASE 1%
			FIND #AP_CLOSE.CH%, KEY #1% &
				GE AP_CLOSE::VENNUM + AP_CLOSE::INVNUM, &
				REGARDLESS
		CASE 2%
			FIND #AP_CLOSE.CH%, KEY #2% &
				GE AP_CLOSE::BATCH + "", &
				REGARDLESS
		END SELECT
	END SELECT
...
28060	!
	...
	AP_CLOSE::TRANKEY = AP_CONTROL::LAST_TRANKEY
