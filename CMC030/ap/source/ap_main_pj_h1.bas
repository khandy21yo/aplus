1	%TITLE "Accounts Payable Purchase Journal Maintenance"
	...
	FUNCTION LONG AP_MAIN_PJ_H1(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	!	.P
	!	This version allows changing the trankey.
	!
	! Index:
	...
	! Option:
	!
	!	AP_MAIN_PJ_H1$LINE_ITEMS
	!
	! Compile:
	!
	!	$ BAS AP_SOURCE:AP_MAIN_PJ_H1/LINE
	...
	!		Fixed bug where F14 on vendor number would only
	!		work when the field was not blank.
	!
	...
	CASE OPT_INIT
		...
		SMG_WINDOW::NHELP = "AP_MAIN_PJ_H1"
		SMG_WINDOW::HSIZE = 78%
		SMG_WINDOW::VSIZE = 18%
		SMG_WINDOW::HVIEW = 78%
		SMG_WINDOW::VVIEW = 18%
...
5000	CASE OPT_MOREMENU
	...
	!	^*Lines Items\*
	!	.p
	!	The ^*Line Items\* option maintains
	!	the records by line.
	!
	! Index:
	!	.x Line Items
	...
	!--
			...
			AP_MAIN_PJ_H1 = MAIN_JOURNAL(AP_MAIN_PJ_L.ID, "")
...
20200	!******************************************************************
 ...
 ELoop:		SCOPE::SCOPE_EXIT = 0%
	...
	!
	!--
			AP_PJH::VENNUM = ENTR_3STRING(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::INVNUM = ENTR_3STRING(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::INVDAT = ENTR_3DATE(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::INVAMT = ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::DESCR = ENTR_3STRING(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::AP_ACCT = ENTR_3STRING(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::DISCDAT = ENTR_3DATE(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::DISCAMT = ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::CODE_1099 = ENTR_3STRING(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::AMT_1099 = ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::CKNUM = ENTR_3STRING(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::CKAMT = ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			AP_PJH::CASH_ACCT = ENTR_3STRING(SCOPE, SMG_WINDOW::WNUMBER, &
...
20300	CASE OPT_TESTENTRY
		...
		CASE 2%
			AP_MAIN_PJ_H1, ST% = FUNC_TESTENTRY(SMG_WINDOW, &
		...
		CASE 3%		! Duplicate Invoice Number
			SCOPE::SCOPE_EXIT = T.SE%
 ...
 DupInv:		IF DUP_INV%
		...
		CASE 7%
			...
			THEN
				AP_MAIN_PJ_H1 = FUNC_TESTENTRY(SMG_WINDOW, &
	...
	CASE OPT_DISPLAY
		...
		THEN
			IF (INSTR(1%, AP_PJH::CASH_ACCT, "?") = 0%) AND &
				(AP_PJH::CKNUM <> "      ")
			THEN
				IF MAIN_WINDOW(GL_MAIN_CHART.ID, "Q0" + AP_PJH::CASH_ACCT) <> 1%
				THEN
					GL_CHART::DESCR = "????????????????????"
				END IF
...
20500	CASE OPT_SETOLD
	...
	CASE OPT_AFTEROPT
		...
		CASE "Add"
			...
			AP_MAIN_PJ_H1 = MAIN_JOURNAL(AP_MAIN_PJ_L.ID, "A")
		...
		CASE "Change"
			...
			THEN
				TEMP$ = AP_PJH::TRANKEY + ""
				AP_PJH = AP_PJH_OLD
				AP_MAIN_PJ_H1 = MAIN_JOURNAL(AP_MAIN_PJ_L.ID, "C" + TEMP$)
			END IF
		...
		CASE "Erase"
			...
			!
			AP_MAIN_PJ_H1 = MAIN_JOURNAL(AP_MAIN_PJ_L.ID, "E")
...
27000	!*******************************************************************
	...
	CASE 2%, 4%
		...
		THEN
			...
			IF MAIN_WINDOW(AP_MAIN_VENDOR.ID, "Q0" + AP_PJH::VENNUM)=1%
			THEN
				...
				ELSE
					...
					NEW.YEAR% = VAL%(MID$(AP_PJH::INVDAT, 1%, 4%))
					IF NEW_MON% > 12%
					THEN
						NEW_MON% = 1%
						NEW.YEAR% = NEW.YEAR% + 1%
 ...
 CheckDate:
			...
			THEN
				...
				CASE ELSE
					NEW_MON% = NEW_MON% + 1%
					NEW_MON% = 1% IF NEW_MON% > 12%
				END SELECT

				AP_PJH::DISCDAT = LEFT(AP_PJH::INVDAT, 4%) + &
					FORMAT$(NEW_MON%, "<0>#") + &
					FORMAT$(NEW_DAY%, "<0>#")
				!
				! Make sure the date exists this month
