1	%TITLE "Create 1099 History File"
	%SBTTL "AP_SPEC_1099_CREATE_TEST"
	%IDENT "V3.6a Calico"
	...
	!	.b
	!	.lm +5
	!	The ^*Create 1099 Work File from History\* option provides the
	!	means to create the subject file by copying the appropriate records
	...
	! Compile:
	!
	!	$ BAS AP_SOURCE:AP_SPEC_1099_CREATE_TEST/LINE
...
330	!
	...
	GL_ENDPERIOD% = GL_PERIOD::FPFY IF GL_ENDPERIOD% < 1%
...
350	SCOPE::PRG_ITEM = "FLD01YEAR"
	...
	!	.b
	!	.lm +5
	!	The ^*Year\* provides a means to identify the 1099 work file.  The
	...
	YEAR_1099$ = LEFT(DATE_TODAY, 2%) + INP$ IF LEN(INP$) = 2%
...
500	!
 ...
 !	END_PERIOD$ = WORK_YEAR$ + FORMAT$(GL_ENDPERIOD%, "<0>#")
	...
	SMG_STATUS% = SMG$PUT_CHARS(SMG_SCREEN_DATA%, &
		...
		END_PERIOD$, 07%, 2%)
	SMG_STATUS% = SMG$PUT_CHARS(SMG_SCREEN_DATA%, "Vendor #",09%, 2%)
...
510	SCOPE::PRG_ITEM = "FLD04PER"
	...
	!	.b
	!	.lm +5
	!	The contents of the ^*Beginning year and Period\* field in the Create 1099
	!	History selection screen ^&must\& be the period of the user's fiscal year
...
520	SCOPE::PRG_ITEM = "FLD05PER"
	...
	!	.b
	!	.lm +5
	!	The contents of the ^*Ending year and period\* field in the Create 1099
	!	History selection screen ^&must\& be the period of the user's fiscal year
...
2025	IF TEST_VENNUM$ <> AP_OPEN::VENNUM
	THEN
		...
		ADD_TO_1099% = -1% IF EDIT$(AP_VENDOR::FLG1099, -1%) = "Y"

	END IF

2030	TEST_VENNUM$ = AP_OPEN::VENNUM

	GOTO 2040 IF ADD_TO_1099% = 0%
...
3030	TEST_VENNUM$ = AP_CLOSE::VENNUM
		...
		THEN
			...
			THEN
				...
				SMG_STATUS% = SMG$PUT_CHARS(SMG_SCREEN_DATA%, &
					...
					AP_CLOSE::VENNUM + " Ck # " + &
					AP_CLOSE::CKNUM + " Date " + &
					PRNT_DATE(AP_CLOSE::CKDAT, 8%) + " Ck Amount " + &
 ...
 CodeMissClose:
			...
			FACTOR = 1.0
			FACTOR = AP_CLOSE::CKAMT / INVAMT IF INVAMT <> 0.0
...
32767	END FUNCTION
	...
	!	.b
	!	.lm +5
	!	The contents of the ^*Beginning year and period\* field in the
	!	Create 1099 History selection screen ^&must\& be the period of the
