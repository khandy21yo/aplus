1	%TITLE "Sales Journal Line Item Maintenance"
	%SBTTL "AR_MAIN_SJ_LINE"
	%IDENT "V3.6a Calico"

	FUNCTION LONG AR_MAIN_SJ_LINE(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	! Compile:
	!
	!	$ BAS AR_SOURCE:AR_MAIN_SJ_LINE/LINE
	...
	!
	!	02/16/92 - Kevin Handy
	!		Removed comma in quanity to give one more digit
	!		in that field (up to 99999.999).
	!
	...
	!		Added type "H" to line codes, for ability to
	!		create records with multiple due dates, and
	!		the description is entered as a date for "H" type.
	!
	!	05/24/95 - Kevin Handy
	!		Removed unecessary externals.
	!
	...
	CASE OPT_INIT
		...
		SMG_WINDOW::NHELP = "AR_MAIN_SJ_LINE"
		SMG_WINDOW::HSIZE = 78%
		SMG_WINDOW::VSIZE = 8%
		SMG_WINDOW::HVIEW = 78%
		SMG_WINDOW::VVIEW = 18%
...
20200	CASE OPT_ENTRY
 ...
 E0Loop:	SCOPE::SCOPE_EXIT = 0%
	...
	!--
			IF AR_SJL::LTYPE = "H"
			THEN
				TEMP$ = LEFT(AR_SJL::DESCR, 8%)
				TEMP$ = ENTR_3DATE(SCOPE, &
					SMG_WINDOW::WNUMBER, &
					XLINE$ + ";33", TEMP$, &
					TEMP$, MFLAG, &
					"8", MVALUE)
...
20300	CASE OPT_TESTENTRY
		...
		CASE 1%
			...
			!
			AR_MAIN_SJ_LINE = FUNC_TESTENTRY( SMG_WINDOW, &
		...
		CASE 2%
			...
			THEN
				IF SB_EXAM_SUBACCOUNT("E", &
						AR_SJL::SUBACCT, &
						SB_SUBACCOUNT_READ) <> CMC$_NORMAL &
					AND SB_EXAM_SUBACCOUNT("J", &
						AR_SJL::SUBACCT, &
						SB_SUBACCOUNT_READ) <> CMC$_NORMAL &
					AND SB_EXAM_SUBACCOUNT("S", &
						AR_SJL::SUBACCT, &
						SB_SUBACCOUNT_READ) <> CMC$_NORMAL
				THEN
