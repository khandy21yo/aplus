1	%TITLE "Accounts Receivable Sales Journal Post Program"
	...
	! Compile:
	!
	!	$ BAS AR_SOURCE:AR_POST_SJFJ/LINE
	!	$ LINK/EXE=AR_EXE: AR_POST_SJFJ, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE AR_POST_SJFJ.OBJ;*
	!
	! Author:
	!
	!	07/12/93 - Kevin Handy
	!		Taken from AR_POST_SJ for Forde Johnson.
	!
	! Modification history:
	!
	!	07/12/93 - Kevin Handy
	!		Modified to print out subtotal by subaccount
	!		for Forde Johnson.
	!
	!	07/12/93 - Kevin Handy
	!		Modified error lines to print when customer
	!		is undefined.
	!
	...
	!		se LIB$DELETE_FILE instead of KILL
	!--
	%PAGE
	...
	RECORD SUBTOTAL_CDD
		STRING	BUYFRAN = 10%	! Location
		STRING	PRODUCT = 18%	! Product
...
3000	WHEN ERROR IN
	...
	!
	TOTAL_DISCOUNT, TOTAL_OTHER = 0.0
	PRINTHEAD% = 0%
	...
	CASE CMC$_UNDEFINED
		...
		CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), &
			"CUSNUM " + AR_SJH::CUSNUM + "UNDEFINED", 0%)
	...
	CASE CMC$_UNDEFINED
		ARACCT$ = "*"
		CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), &
			"ACCOUNT " + AR_SJH::ARACCT + "UNDEFINED", 0%)
...
3200	WHEN ERROR IN
	...
	CASE ELSE
		ACCTNUM$ = "*"
		CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), &
			"ACCOUNT " + AR_SJL::ACCT + "UNDEFINED", 0%)
		GOTO Aborted
	...
	CASE CMC$_WARNING
		...
		CASE ELSE
			SUBACCT$ = "*"
			CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), &
				"SUBACCOUNT " + AR_SJL::SUBACCT + "UNDEFINED", 0%)
 ...
 GoToNext:
	...
	CASE "D"
		TOTAL_DISCOUNT = TOTAL_DISCOUNT + AR_SJL::AMOUNT

	CASE "O", "F", "T"
		TOTAL_OTHER = TOTAL_OTHER + AR_SJL::AMOUNT
...
5020 !	WHEN ERROR IN
 ...
 AddSubtotal:
	ACCTNUM$ = GL_YYYY_PP::ACCT + ""
	Z% = INSTR(1%, ACCTNUM$, ".")
	LSET ACCTNUM$ = LEFT(ACCTNUM$, Z% - 1%) IF Z%

	FOR I% = 1% TO SUBTOTAL%
		GOTO AddSubtotal1 &
			IF (SUBTOTAL(I%)::BUYFRAN = GL_YYYY_PP::SUBACC) AND &
			(SUBTOTAL(I%)::PRODUCT = ACCTNUM$)

		GOTO AddSubtotal2 &
			IF (SUBTOTAL(I%)::BUYFRAN + SUBTOTAL(I%)::PRODUCT) > &
			GL_YYYY_PP::SUBACC + ACCTNUM$
	NEXT I%
	I% = SUBTOTAL% + 1%
 ...
 AddSubtotal2:
	...
	SUBTOTAL% = SUBTOTAL% + 1%
	SUBTOTAL(I%)::BUYFRAN = GL_YYYY_PP::SUBACC
	SUBTOTAL(I%)::PRODUCT = ACCTNUM$
 ...
 AddSubTotal1:
	...
	SUBTOTAL(I%)::QUANTITY = SUBTOTAL(I%)::QUANTITY + GL_YYYY_PP::UNITS
	SUBTOTAL(I%)::PRICE = SUBTOTAL(I%)::PRICE + GL_YYYY_PP::AMOUNT
...
32000	!******************************************************************
	! End of posting program AR_POST_SJFJ
	!******************************************************************
	END
