1	%TITLE "Product Cost Structure"
	...
	! Compile:
	!
	!	$ BAS BM_SOURCE:BM_RPRT_MINMAX_LOWCOST/LINE
	!	$ LINK/EXE=BM_EXE: BM_RPRT_MINMAX_LOWCOST, FUNC_LIB:CMCLINK/OPTION
 ...
 Init:	! Initilize report
	...
	DISPLAY_DETAIL$ = EDIT$(UTL_REPORTX::OPTDEF(8%), 132%)

	!++
	! Abstract:FLD09
	!	^*(09) Display Detail (Y,N)\*
	!	.b
	!	.lm +5
	!	The ^*Display Detail\* field indicates if the report is
	!	to print a summary only or is to print a detailed report.
	!	.b
	!	Valid choices are:
	!	.table 3,25
	!	.te
...
600	!
 ...
 ReportTitle:
	...
	IF DISPLAY_DETAIL$ = "Y"
	THEN
		TITLE$(5%) = SPACE$(17%) + "Operation       Hours " + &
			"     LaborCost  Type Description     " + &
			"       MaterialCost"

		TITLE$(6%) = SPACE$(26%) + "Type Component#     " + &
			"Description                              UOM     " + &
...
17020	!
	...
	CASE "G"
		GOTO ExitProgram IF (BM_MAXMIN::MGROUP > TO_ITEM$) AND &
			TO_ITEM$ <> ""
		...
		IF TEST_CATEGORY$ <> BM_MAXMIN::MGROUP AND &
		...
		TEST_CATEGORY$ = BM_MAXMIN::MGROUP

	CASE "P"
		GOTO ExitProgram IF (BM_MAXMIN::PRODUCT > TO_ITEM$) AND &
			TO_ITEM$ <> ""
...
17500	IF BM_CONTROL::BURDENPERC <> 0.0
	...
	GOTO 17700 IF DISPLAY_DETAIL$ <> "Y"
	...
	!
	! Get other info to print
	!
	IF TYPE_LOOP% > LABOR_LOOP%
	THEN
		PRINT_LOOP% = TYPE_LOOP%
	ELSE
		PRINT_LOOP% = LABOR_LOOP%
	END IF

	FOR I% = 1% TO PRINT_LOOP%
...
17600		WHEN ERROR IN
			GET #PD_PRODTYPE.CH%, &
				...
				REGARDLESS
		USE
			CONTINUE 17610 IF ERR = 155% OR ERR = 9%
			FILENAME$ = "PD_PRODTYPE"
			CONTINUE HelpError
		END WHEN

17610		LABOR = FUNC_ROUND(HOURS(I%) * RATE(I%), 3%)
		...
		TEXT$ = TEST_PRODUCT(1%) + "   " + &
			...
			PRODTYPE(I%) + "   " + &
			PD_PRODTYPE::DESCRIPTION + " " + &
			FORMAT$(PRODTYPEAMT(I%), "<%>#,###,###.###")

		CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), TEXT$, 0%)
		GOTO ExitProgram IF UTL_REPORTX::STAT

		TOTAL_HOURS    = TOTAL_HOURS + HOURS(I%)
		TOTAL_LABOR    = TOTAL_LABOR + LABOR
		TOTAL_MATERIAL = TOTAL_MATERIAL + PRODTYPEAMT(I%)

	NEXT I%

	TEXT$ = TEST_PRODUCT(1%) + SPACE$(01%) + &
		"TOTALS:" + SPACE$(04%) + &
		FORMAT$(TOTAL_HOURS, "<%>###,###.###") + SPACE$(01%) + &
		FORMAT$(TOTAL_LABOR, "<%>#,###,###.###") + SPACE$(28%) + &
		FORMAT$(TOTAL_MATERIAL, "<%>#,###,###.###")
	...
	FOR I% = 1% TO ITEM_LOOP%
		...
		TOTAL_COMPONENT = FUNC_ROUND(COMPONENT(I%)::QUANTITY * COST, 3%)
