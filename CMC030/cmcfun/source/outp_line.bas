1	%TITLE "Handle Paging on Reports"
	...
	!		Use A"x"B
	!
	!	02/15/2000 - Kevin Handy
	!		Make output to spreadsheet include full lines
	!		when the layout is passed as a blank string.
	!
...
2050		!
		...
		PRINT #UTL_REPORTX::CHAN
		PRINT #UTL_REPORTX::CHAN, &
			FNOFFSET$(STRING$(UTL_REPORTX::REPWIDTH, A"="B), &
			UTL_REPORTX::OFFSET)
		...
		FOR JUNK1% = JUNK1% + 1% UNTIL HDR(JUNK1%) = ""
			...
			JUNK$ = "" IF JUNK$ = "."
			PRINT #UTL_REPORTX::CHAN, &
				FNOFFSET$(JUNK$, UTL_REPORTX::OFFSET)
			UTL_REPORTX::LINENO = UTL_REPORTX::LINENO + 1%
		NEXT JUNK1%
...
3000	!
	...
	CASE 1%
		TEXT$ = "Date: " + DATE$(0%)
		TEXT$ = TEXT$ + &
			SPACE$(UTL_REPORTX::REPWIDTH / 2% - &
				LEN(TEXT$) - &
				LEN(JUNK$) / 2%) + &
			JUNK$
		TEXT$ = TEXT$ + &
			SPACE$(UTL_REPORTX::REPWIDTH - &
				LEN(TEXT$) - &
				LEN(JUNK3$)) + JUNK3$
		PRINT #UTL_REPORTX::CHAN, &
			FNOFFSET$(TEXT$, UTL_REPORTX::OFFSET)
