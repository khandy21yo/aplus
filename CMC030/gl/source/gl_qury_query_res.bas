1	%TITLE "QUERY - Query General Ledger System"
	...
	! Option:
	!
	!	GL_QURY_QUERY_RES$PRINT
	!
	! Compile:
	!
	!	$ BAS GL_SOURCE:GL_QURY_QUERY_RES/LINE
	!	$ LINK/EXECUTABLE=GL_EXE: GL_QURY_QUERY_RES, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE GL_QURY_QUERY_RES.OBJ;*
	!
	! Author:
	!
	!	08/20/97 - Kevin Handy
	!		Based upon GL_QURY_QUERY, and modified to allow
	!		seing details only on a defined list of accounts.
	!
	...
	DIM RANGE.CH%(MAX.RANGE)
	DIM GL_YYYY_PP_CDD RANGE.BUF(MAX.RANGE)
...
330	!
	! Open table of valid accounts for this user
	!
	WHEN ERROR IN
	...
	WHILE GL_USERLIST::USER = USERNAME$

		ACCOUNTLIST$ = ACCOUNTLIST$ + "," + TRM$(GL_USERLIST::ACCOUNT)
...
340	!
	! Do we have any valid accounts
	!
	IF ACCOUNTLIST$ = ""
	THEN
		CALL ENTR_3MESSAGE(SCOPE, "No valid accounts known", 0%)
		GOTO ExitProgram
	END IF
...
1100	!
	...
	!	accesses a report setting screen, which
	!	prints the specific information
	!	being queried.
	!	.lm -5
	!
	! Index:
 ...
 Changer:
		...
		LOOP% = ENTR_3NUMBER(SCOPE, SCOPE::SMG_OPTION, "", "Item to change", &
			0.0, 4%, "##", "")
...
4200		!
		...
		FOR COUNTER% = FROM_PERIOD% TO TO_PERIOD%
			...
			THEN
				SELECT BY_ITEM$
				CASE "A"
					ITEM$ = RANGE.BUF(COUNTER%)::ACCT
				CASE "S"
					ITEM$ = RANGE.BUF(COUNTER%)::SUBACC
				CASE "X"
					ITEM$ = RANGE.BUF(COUNTER%)::XREFNO
				CASE "C"
					ITEM$ = RANGE.BUF(COUNTER%)::CKNO
				CASE "B"
					ITEM$ = RANGE.BUF(COUNTER%)::BTHNUM
				END SELECT
...
4320		!
		...
		!
		! Don't let them see it if they are not allowed to
		!
		GOTO 4340 &
			IF INSTR(1%, ACCOUNTLIST$, TRM$(GL_YYYY_PP::ACCT)) <= 1%
...
4340		WHEN ERROR IN
		...
		USE
			CLOSE RANGE.CH%(COUNTER%) IF ERR = 11%
			CALL ASSG_FREECHANNEL(GL_YYYY_PP.CH%)
			RANGE.CH%(COUNTER%) = -1% IF ERR = 11%
			CONTINUE 4380
		END WHEN
...
4400		!
 ...
 DataEntry2a:
	...
	!
	!--
		BY_ITEM$ = EDIT$(ENTR_3STRING(SCOPE, SMG_QUERY%, "6;38", "By item", &
			BY_ITEM$, FLAG%, "'E", DEFLT$), -1%)
