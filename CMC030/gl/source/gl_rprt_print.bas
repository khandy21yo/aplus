1	%TITLE "GENLED - Print the General Ledger"
	...
	! Compile:
	!
	!	$ BAS GL_SOURCE:GL_RPRT_PRINT/LINE
	...
	!		Added account number (and begin balance)
	!		to output to the 2020 file.
	!
	!	09/10/90 - Kevin Handy
	!		Fixed bug in Annual General Ledger, where CUR_PERIOD%
	!		wasn't being handled very well.
	!
	!	09/10/90 - Kevin Handy
	!		Moved code to initialize period out of select
	!		statement to make it easier to read.
	!
	!	09/19/90 - Kevin Handy
	!		Added intermediate totals in annual GL.
	!
	!	01/10/91 - Craig Tanner
	!		Where FILENAME$ = "GL_YYYY_PP" in error handler,
	!		changed to = "GL_" + YYYY_PP$.
	!
	!	09/12/91 - Kevin Handy
	!		Modified so that if you are printing an annual GL,
	!		and you are missing periods, then it will still
	!		print what it can.
	!
	!	10/05/92 - Kevin Handy
	!		Modified annual GL to load  summary total from a
	!		long loop instead of relying on the summary total
	!		stored in the period file which has proven to
	!		be slightly unreliable.
	!
	!	06/13/93 - Kevin Handy
	!		Added REGARDLESS to GL_CHART.
	!
	!	07/07/93 - Frank F. Starman
	!		Fixed print by summary flag.
	!
	!	10/04/93 - Kevin Handy
	!		Modified to use FORMAT$ instead of RIGHT(NUM1$(
	!		business.
	!
	!	04/15/94 - Kevin Handy
	!		Modified to print out the summaries between the
	!		subtotals, not after the last subtotal.
	!
	!	04/12/95 - Kevin Handy
	!		(V3.6)
	!		Update to V3.6 Calico coding standards.
	!		Reformat closer to 80 cols (looking for LL bug).
	!
	...
	!		Don't need to assign UTL_REPORTX::CHAN ourselves.
	!
	...
	!		Increased dimension of arrays from 300 to 2500 (FJ)
	!
	!	05/17/2000 - Kevin Handy
	!		Use WHEN ERROR IN
	!
	!	07/15/2003 - Kevin Handy
	!		Clean up source.
	!--
	%PAGE
	...
	! Dimension arrays
	!
	DIM LONG GL_YYYY_PP.CH(13%), GL_YYYY_PP_EOF(13%), TRAN_COUNT(2500%)
	DIM REAL SUMMARY_TOTAL(2500%)
	DIM STRING TRANDAT(2500%), SOURCE(2500%), BATCH_ID(2500%), &
...
18000	!******************************************************************
	...
	IF ZERO_BALANCE$ <> "N" OR BEG_BAL <> 0.0
	THEN
		PRINT_BEGBAL% = -1%
		GOSUB PrintBegBal
	...
	FOR LOOP% = 1% TO CHART_LOOP% + 1%
		...
		WHEN ERROR IN
			FIND #GL_YYYY_PP.CH(LOOP%), &
				...
				REGARDLESS
		USE
			GL_EMPTY_FLAG%, GL_YYYY_PP_EOF(LOOP%) = -1% &
				IF ERR = 11%
			CONTINUE 18020
		END WHEN
