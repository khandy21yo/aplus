1	%TITLE "001 - Print a Previous General Ledger"
	%SBTTL "GL_RPRT_PRINT_PERIOD"
	%IDENT "V3.6a Calico"
	...
	!	.b
	!	.lm +5
	!	The ^*Previous General Ledger Report\* is a
	!	listing of transactions by
	!	account number in date order, with resulting balances
	!	for a previous period.
	!	This allows you to reprint a ^*General Ledger\*
	!	after additional periods have been closed.
	!	.b
	...
	! Compile:
	!
	!	$ BAS GL_SOURCE:GL_RPRT_PRINT_PERIOD/LINE
	...
	! Author:
	!
	!	07/21/97 - Kevin Handy
	!		Based upon GL_RPRT_PRINT
	!
 ...
 Initialization:
	...
	PRINT_PERIOD$ = LEFT(UTL_REPORTX::OPTDEF(5%), 6%)
	YYYY_PP$ = LEFT(PRINT_PERIOD$, 4%) + "_" + RIGHT(PRINT_PERIOD$, 5%)
	...
	!	.b
	!	.lm +5
	!	Specifies which period to print
	!	.lm -5
	!
	! Index:
	!	.x General Ledger Report>Period
	!
	!--
...
300	!
	...
	!
	! Calculate which element they really want to display
	!
	CLOSED_PERIOD% = GL_PERIOD::LASTPERCLO + &
		VAL%(GL_PERIOD::YEAR) * GL_PERIOD::FPFY
	WANTED_PERIOD% = VAL%(RIGHT(YYYY_PP$, 6%)) + &
		VAL%(LEFT(YYYY_PP$, 4%))  * GL_PERIOD::FPFY
	CHART_LOOP% = CLOSED_PERIOD% - WANTED_PERIOD% + 3%

	IF (CHART_LOOP% < 0%) OR (CHART_LOOP% > 20%)
	THEN
		CALL HELP_3MESSAGE(SCOPE, "PERIOD " + YYYY_PP$ + &
			" IS NOT AVAILABLE", "ERR", "GL_PERIOD", "NO_PERIOD")
		GOTO ExitProgram
	END IF
...
330	!
 ...
 ReportTitle:
	...
	TITLE$(TITLE% + 1%) = "Period General Ledger"
...
17900	!******************************************************************
	...
	THEN
		...
		GOTO 17900 IF GL_YYYY_PP_EOF% = 0%
	END IF
...
32767	!******************************************************************
	! End of report GL_RPRT_PRINT_PERIOD
	!******************************************************************
	END
