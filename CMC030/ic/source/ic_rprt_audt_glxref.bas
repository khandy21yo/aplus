1	%TITLE "IG to GL Comparison Report"
	...
	! Compile:
	!
	!	$ BAS IC_SOURCE:IC_RPRT_AUDT_GLXREF/LINE
	...
	! Author:
	!
	!	03/12/2002 - Kevin Handy
	!		Based on IC_RPRT_AUDT_GL
	!
	...
	! Declare constants
	!
	DECLARE	LONG	CONSTANT MAX_BATCH = 800
	DECLARE LONG	CONSTANT MAX_GRAND = 800
 ...
 Initialization:
	...
	FROM_ITEM$ = LEFT(EDIT$(UTL_REPORTX::OPTDEF(0%), 32%), 6%)
...
348	CLOSE #UTL_TRANSTYPE.CH%
 ...
 ReportTitle:
	...
	TITLE$(4%) = "                                                 " + &
		"  ------------General Ledger-------------" + &
		" --------------Inventory----------------"
	TITLE$(5%) = "Account #          XRef       Description       " + &
		"          Debit         Credit       Units" + &
		"        Debit         Credit       Units"
	TITLE$(6%) = ""
...
17020	GL_YYYY_PP::BTHNUM = "ZZZZZZ"
...
17110	!
	...
	IF SORTBY$ <> "X"
	THEN
		TEST_BATCH$ = GL_YYYY_PP::XREFNO
	ELSE
		TEST_BATCH$ = GL_YYYY_PP::BTHNUM
	END IF

	WHILE STO% - SFROM% > 4%
		SMID% = (SFROM% + STO%) / 2%
		IF (BATCH_TOTAL(SMID%)::ACCT + BATCH_TOTAL(SMID%)::XREF <= &
			GL_YYYY_PP::ACCT + TEST_BATCH$)
		THEN
			SFROM% = SMID%
		ELSE
			STO% = SMID%
	...
	!
	GOTO GotAccount &
		IF (BATCH_TOTAL(I%)::ACCT + BATCH_TOTAL(I%)::XREF = &
		GL_YYYY_PP::ACCT + TEST_BATCH$) &
		FOR I% = SFROM% TO STO%
	...
	WHILE (I% > 1%) AND &
		(BATCH_TOTAL(I% - 1%)::ACCT + BATCH_TOTAL(I% - 1%)::XREF > &
		GL_YYYY_PP::ACCT + TEST_BATCH$)
...
17210	!
	...
	IF SORTBY$ <> "X"
	THEN
		TEST_BATCH$ = IC_TRANSACTION::CROSS_REF
	ELSE
		TEST_BATCH$ = IC_TRANSACTION::BATCH
	END IF

	WHILE STO% - SFROM% > 4%
		SMID% = (SFROM% + STO%) / 2%
		IF (BATCH_TOTAL(SMID%)::ACCT + BATCH_TOTAL(SMID%)::XREF <= &
			IC_ACCOUNT$ + TEST_BATCH$)
		THEN
			SFROM% = SMID%
		ELSE
			STO% = SMID%
...
18020		IF ABS(BATCH_TOTAL(I%)::DEBIT - &
			BATCH_TOTAL(I%)::ICDEBIT) > 0.001 OR &
			ABS(BATCH_TOTAL(I%)::CREDIT - &
			BATCH_TOTAL(I%)::ICCREDIT) > 0.001
		THEN
			TEST_FLAG$ = "******"
			ONE_FLAG$ = "**"
		ELSE
...
32767	!******************************************************************
	! End of report IC_RPRT_AUDT_GLXREF
	!******************************************************************
	END
