1	%TITLE "Product Location Maximum Level Balances"
	%SBTTL "IC_RPRT_MAXLEVEL"
	%IDENT "V3.6a Calico"
	...
	!	.b
	!	.lm +5
	!	The ^*Maximum Level Balances\* report compares the current balance with what
	!	the maximum balance should be. This report is necessary for storing purposes and
	!	is printed by location. The report contains the following fields:
	!	.table 3,25
	!	.te
	...
	!	Type	Category
	!	.te
	!	Maximum Level	On Hand
	!	.te
	!	Difference Sales Order	Allocated
	!	.te
	!	On Order
	!	.end table
	!	.lm +5
	!
	! Index:
	!	.x Maximum Level>Report
	!
	! Option:
	!
	! Compile:
	!
	!	$ BAS IC_SOURCE:IC_RPRT_MAXLEVEL/LINE
	!	$ LINK/EXE=IC_EXE: IC_RPRT_MAXLEVEL, FUNC_LIB:CMCLINK/OPTION
	...
	!
	!	11/01/91  - Dan Perkins
	!		Cleaned up progam output.  Balances -
	!		Allocated, and On Order print on the
	!		same line as rest of report.
	!
	!	01/16/92 - Dan Perkins
	!		Changed quantity values to display integer values.
	!
 ...
 Init:	!
	...
	!
	! Index:
	!	.x From Item>Maximum Level Balances
	!	.x Maximum Level Balances>From Item
	!	.x Item>From
	!
	!--
	...
	!
	! Index:
	!	.x To Item>Maximum Level Balances
	!	.x Maximum Level Balances>To Item
	!	.x Item>To
	!
	!--
	...
	!
	! Index:
	!	.x Wildcard>Maximum Level Balances
	!	.x Maximum Level Balances>Wildcard
	!
	!--
	...
	!
	! Index:
	!	.x Locations>Maximum Level Balances
	!	.x Maximum Level Balances>Locations
	!
	!--
	...
	!	.b
	!	.lm +5
	!	The ^*Only Beginning\* field prints
	!	the report containing only the beginning of the period information
	!	or all information including the present period.
	!	.b
	!	Valid entries are:
	!	.table 3,25
	!	.te
	...
	!++
	! Abstract:FLD07
	!	^*(07) Only Over Maximum\*
	!	.b
	!	.lm +5
	!	The ^*Only Over Maximum\* field prints
	!	all balances or only those balances which are over the maximum.
	!	A ^*N\* answer prints all balances while a ^*Y\* answer
	!	prints only the over maximum balances.
	!	.lm -5
	!
	! Index:
	!	.x Only Over Maximum
	...
	PRINT_BAL$ = EDIT$(UTL_REPORTX::OPTDEF(7%), 132%)
	...
	!	.b
	!	.lm +5
	!	The ^*Print Balances (Y/N)\* option prints
	...
	!
	! Index:
	!	.x Print Balances>Maximum Level Balances
	!	.x Maximum LEvel Balances>Print Balances
	!
	!--
	...
	!	.b
	!	.lm +5
	!	The ^*Sort by\* field processs the
	...
	!
	! Index:
	!	.x Sort by>Maximum Level Balances
	!	.x Maximum Level Balances>Sort By
	!
	!--
...
330	WHEN ERROR IN
 ...
 ReportTitle:
	...
	! Title
	!
	TITLE$(1%) = "PRODUCT  MAXIMUM  LEVEL  " + ADD_TITLE$
	...
	LYT_LINE$ = "$Product#:015,$Description:047,$Tp:050,$Cat:059,$MaxLevel:074,VOnHand:089,VDiff:105"
...
17300	!
	...
	IF ONLY_CTD$ = "N"
	THEN
		ONHAND = ONHAND + BALANCE(1%, 2%)
		ALLOC = ALLOC + BALANCE(2%, 2%)
		ONORDER = ONORDER + BALANCE(3%, 2%)
	END IF

	DIFF = IC_BINMAP::MAXLEVEL - ONHAND

	IF (ONLY_NEG$ = "Y" AND DIFF < 0.0) OR (ONLY_NEG$ = "N")
	THEN
		IF PRINT_BAL$ <> "Y"
		THEN
