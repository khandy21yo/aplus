1	%TITLE "Product Ordering Report"
	...
	! Compile:
	!
	!	$ BAS IC_SOURCE:IC_RPRT_PRODORDER/LINE
	!	$ LINK/EXE=IC_EXE: IC_RPRT_PRODORDER, FUNC_LIB:CMCLINK/OPTION
	...
	!		Made program fit on scope screen per client request.
	!
	!	06/15/92 - Kevin Handy
	!		Clean up (check)
	!
	!	09/15/92 - Dan Perkins
	!		Added product status wildcard to print selected types
	!		of product status like active, or inactive.
	!
	!	09/21/92 - Kevin Handy
	!		Clean up (check)
	!
	!	01/25/94 - Kevin Handy
	!		Split "ExitProgram" into "ExitTotal" and "ExitProgram"
	!		so programs based off this one aren't so hard to put
	!		totals in.
	!
...
330	WHEN ERROR IN
 ...
 ReportTitle:
	...
	TITLE$(5%) = "Product#       Description          " + &
		"Tp Cat  SecCode    LostSal     Qtr5     Qtr4  " + &
