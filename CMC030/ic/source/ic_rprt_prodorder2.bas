1	%TITLE "Product Ordering Report"
	...
	! Compile:
	!
	!	$ BAS IC_SOURCE:IC_RPRT_PRODORDER2/LINE
	!	$ LINK/EXE=IC_EXE: IC_RPRT_PRODORDER2, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE IC_RPRT_PRODORDER2.OBJ;*
	!
	! Author:
	!
	!	09/14/94 - Kevin Handy
	!		Taken from IC_RPRT_PRODORDER and modified
	!
	...
	!		Reformatted so that product number isn't on a
	!		seperate line from the other information.
	!
	!	01/04/98 - Kevin Handy
	!		Add long/short product number option.
	!
	...
	!		Fix LCOATION in title so it isn't blank
	!--
	%PAGE
 ...
 Init:	!
	...
	SHORT_PROD$ = LEFT$(UTL_REPORTX::OPTDEF(6%), 1%)

	!++
	! Abstract:FLD07
	!	^*(07) Short/Long Product Number\*
	!	.b
	!	.lm +5
	!	This field will allow you to decide if you want to see
	!	a longer part number without the type or category fields,
	!	or a shorter part number with those fields.
	!	.lm -5
	!
	! Index:
	!	.x Product Number>Product Ordering Report
	!	.x Product Ordering Report>Product Number
	!
	!--
 ...
 ReportTitle:
	...
	THEN
		TITLE$(5%) = "Product#           Description          " + &
			" Tp  SecCode    LstSal     Qtr5     Qtr4  " + &
			"   Qtr3     Qtr2     QCur OnOrder  OnHand   Alloc"
	ELSE
		TITLE$(5%) = "Product#       Description          " + &
			" Tp Cat  SecCode    LstSal     Qtr5     Qtr4  " + &
			"   Qtr3     Qtr2     QCur OnOrder  OnHand   Alloc"
	END IF
