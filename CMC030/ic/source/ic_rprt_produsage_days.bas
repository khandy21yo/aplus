1	%TITLE "Product Days Availability"
	%SBTTL "IC_RPRT_PRODUSAGE_DAYS"
	%IDENT "V3.6a Calico"
	...
	!	.b
	!	.lm +5
	!	The ^*Product Days Availability Report\* is used to estimate
	!	the number of days worth of inventory products are
	!	currently on hand.
	!	fields:
	!	.table 3,25
	!	.te
	...
	!	.te
	!	On Hand	Days Available
	!	.end table
	!	.b
	!	The annual days available columns are based on the
	!	entire previous years usage (Quarters 2 through 5).
	!	.b
	!	The quarter days available columns are based on
	!	this quarter last year (Quarter 5)
	!	.lm -5
	!
	! Index:
	!	.x Product Days Availability>Report
	!	.x Report>Product Days Availability
	!
	! Option:
	!
	! Compile:
	!
	!	$ BAS IC_SOURCE:IC_RPRT_PRODUSAGE_DAYS/LINE
	!	$ LINK/EXE=IC_EXE: IC_RPRT_PRODUSAGE_DAYS, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE IC_RPRT_PRODUSAGE_DAYS.OBJ;*
	!
	! Author:
	!
	!	10/15/2002 - Kevin Handy
...
320	!
 ...
 ReportTitle:
	...
	TITLE$(6%) = "Product#       Description          " + &
		"Tp Cat     Qtr5    Qtr4 " + &
		"   Qtr3    Qtr2    QCur  OnHand " + &
		"Units/Day DaysAvil Units/Day DaysAval"
...
17130	NEXT Y%
	...
	THEN
		...
		! Annual average
		!
		USAGE = ABS(QTYQTR(START% - 1%) + &
			QTYQTR(START% - 2%) + &
			QTYQTR(START% - 3%) + QTYQTR(START% - 4%)) / 365.0
