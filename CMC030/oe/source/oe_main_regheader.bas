1	%TITLE "Order Entry Register"
	...
	!		Remove overwriting ship to address.
	!
	!	01/24/94 - Kevin Handy
	!		Clean up (Check)
	!
	!	04/04/94 - Kevin Handy
	!		Re-enabled the other keys in the OPT_INIT
	!		so that the list of keys there matched
	!		those in OPT_FIND and in the open statement.
	!
	...
	!		Fix view so things fit in the boxes again
	!
	!	11/01/2000 - Kevin Handy
	!		Use A"x"B
	!		Use WHEN ERROR IN
	!
	!	11/19/2001 - Kevin Handy
	!		Put an EDIT$() on CUSTPO in view (KBJ)
	!--
	...
	CASE OPT_INIT
		...
		! Define window
		!
		SMG_WINDOW::DESCR = "Order-Invoice Register"
...
20200	!******************************************************************
 ...
 Reenter:	SCOPE::SCOPE_EXIT = 0%
	...
	!	at the time the order was originally entered.
	!	.b
	...
	!	.b
	!	.lm +5
	!	The ^*Date\* field enters with the date the order was produced.
	!	.b
	...
	!--
			...
			OE_REGHEADER::LOCATION = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"18;18", TEMP$, OE_REGHEADER::LOCATION, MFLAG, &
				"'E", MVALUE)
	...
	!	.b
	!	.lm +5
	!	The ^*Amount Paid\* field contains the amount of the payment.
	!	.lm -5
	!
	! Index:
	!
	...
	!--
			OE_REGHEADER::AMTPAID = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"11;18", TEMP$, OE_REGHEADER::AMTPAID, &
				MFLAG, "###,###.##", MVALUE)
	...
	!--
 FirstNote:
			OE_REGHEADER::NOTES(0%) = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::LWINDOW, &
 ...
 SecondNote:
			OE_REGHEADER::NOTES(1%) = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::LWINDOW, &
			...
			OE_REGHEADER::NOTES(2%) = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::LWINDOW, &
...
20500	CASE OPT_SETOLD
	...
	CASE OPT_VIEW
		...
		CASE 3%
			CUSTPO$ = EDIT$(OE_REGHEADER::CUSTPO, 4%) + SPACE$(10%)
