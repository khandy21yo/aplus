1	%TITLE "Shipped Items by Product Report"
	...
	! Compile:
	!
	!	$ BAS OE_SOURCE:OE_RPRT_ONORDER/LINE
	!	$ LINK/EXE=OE_EXE: OE_RPRT_ONORDER, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE OE_RPRT_ONORDER.OBJ;*
	!
	! Author:
	!
	!	08/16/1999 - Kevin Handy
	!		Based on OE_RPRT_SHIPPROD
	!
...
315	!
 ...
 ReportTitle:
	...
	CASE "P"
		K_NUM% = 0%
		TITLE$(1%) = "ON ORDER ITEMS BY PRODUCT NUMBER"

	CASE "T"
		K_NUM% = 1%
		TITLE$(1%) = "ON ORDER ITEMS BY PRODUCT TYPE"

	CASE "C"
		K_NUM% = 2%
		TITLE$(1%) = "ON ORDER ITEMS BY PRODUCT CATEGORY"

	CASE "D"
		K_NUM% = 3%
		TITLE$(1%) = "ON ORDER ITEMS BY PRODUCT DESCRIPTION"
	END SELECT
	...
	CASE "YN"
		TITLE$(5%) = "Product        Description          " + &
			"PT PCat  QtyOnO ShpDate  CusName" + &
			"              Order#         ExtPrice"
...
17120	WHEN ERROR IN
	...
	!	Lose this line to get all on-orders
	!
