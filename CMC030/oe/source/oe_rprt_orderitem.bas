1	%TITLE "Ordered Items by Product Report"
	...
	! Compile:
	!
	!	$ BAS OE_SOURCE:OE_RPRT_ORDERITEM/LINE
	!	$ LINK/EXE=OE_EXE: OE_RPRT_ORDERITEM, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE OE_RPRT_ORDERITEM.OBJ;*
	!
	! Author:
	!
	!	10/09/91 - Dan Perkins
	!
	! Modification History:
	!
	!	10/14/91 - Dan Perkins
	!		Added Extended Price, Cost, and
	!		Gross Margin columns to report.
	!		Added entry option to display these fields.
	!
	!	05/27/92 - Dan Perkins
	!		Added product SECONDARY CODE to sort option.
	!
	...
	!
	!	07/15/99 - Kevin Handy
	!		Use current cost on everything, instead of the
	!		cost in the archives.
	!
 ...
 Init:	! Initilize report
	...
	!	.b
	!	.lm +5
	!	The ^*Show Totals\* field determines if
	!	the report is to print the total cost and gross MARGIN_
	!	.table 3,25
	!	.te
	!	^*Y\* - Yes
	!	.te
...
315	!
 ...
 ReportTitle:
	...
	CASE "NY"
		TITLE$(5%) = "Product        Description          " + &
			"                    PT PCat  QtyOrd"  + &
			"     ExtPrice      TotCost       Margin"

	CASE "YN"
		TITLE$(5%) = "Product        Description          " + &
			"PT PCat  QtyOrd OrdDate  CusName"     + &
			"              Order#         ExtPrice"
...
17120	WHEN ERROR IN
	...
	IF OE_REGHEADER_READ::ORDDATE >= FROM_DATE$  AND &
		OE_REGHEADER_READ::ORDDATE <= TO_DATE$
	THEN
		...
		UNIT_COST = PC_READ_COST( &
			...
			DATE_TODAY, "")
		COST = FUNC_ROUND(OE_REGLINE::QTY * UNIT_COST, 2%)
