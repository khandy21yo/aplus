1	%TITLE "Salesman by Location Report"
	%SBTTL "OE_RPRT_SALESMAN"
	%IDENT "V3.6a Calico"
	...
	!	(5) - Fifth
	!	.te
	!	(A) - All backorders may be selected.
	!	.end table
	!	The report lists
	!	the items in date sequence (earliest to most recent) and will reflect
	...
	! Compile:
	!
	!	$ BAS OE_SOURCE:OE_RPRT_SALESMAN/LINE
	!	$ LINK/EXE=OE_EXE: OE_RPRT_SALESMAN, FUNC_LIB:CMCLINK/OPTION
 ...
 Init:	! Initilize report
	...
	!	.b
	!	.lm +5
	!	The ^*Age Code\* allows printing the report
...
350	CALL ENTR_3MESSAGE(SCOPE, "Creating temporary file...", 1% + 16%)
	...
	OPEN "OE_TEMP.TMP" FOR OUTPUT AS FILE #OE_TEMP.CH%, &
		...
		MAP OE_TEMP, &
		PRIMARY KEY (OE_TEMP.DATE$, OE_TEMP.ORDER$) DUPLICATES, &
		TEMPORARY, &
		BUFFER 32%, &
...
400	WHEN ERROR IN
 ...
 NextRegline:
430	GOTO NextHeader IF OE_READ_REGLINE(OE_REGHEADER::ORDNUM, LINE$, "GT", &
		OE_REGLINE_READ, QTY()) <> CMC$_NORMAL

	LINE$ = OE_REGLINE_READ::LIN

	IF QTY(7%) > 0.0	! Test for backorder
	THEN
 ...
 ReportTitle:
	...
	! Heading
	!
	TITLE$(5%) = "ExpShipDat Late CusNum      DocNumber   Lin" + &
		"e  ProductCode    Description           " + &
		"        Price   BackOrderQty   Broker  "  + &
		" Operator"
