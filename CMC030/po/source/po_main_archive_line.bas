1	%TITLE "Maintain Purchase Archive Sub Lines"
	%SBTTL "PO_MAIN_ARCHIVE_LINE"
	%IDENT "V3.6a Calico"

	FUNCTION LONG PO_MAIN_ARCHIVE_LINE(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	!	maintains the PO Archive sub lines.
	!	.lm -5
	!
	! Index:
	!
	...
	! Compile:
	!
	!	$ BAS PO_SOURCE:PO_MAIN_ARCHIVE_LINE/LINE
	...
	COM (CH_PO_REG_SUB_LINE) &
		...
		PO_REG_SUB_LINE.CH_ARC%, &
		PO_REG_SUB_LINE.READONLY_ARC%
...
700		!
		...
		THEN
			...
			SMG_WINDOW::FLAGS = (SMG_WINDOW::FLAGS OR 2%) &
				IF PO_REG_SUB_LINE.READONLY_ARC%
			GOTO 790
		END IF
		...
		!
750		WHEN ERROR IN
			%INCLUDE "SOURCE:[PO.OPEN]PO_REG_SUB_LINE.ARC"
		USE
		...
		PO_REG_SUB_LINE.READONLY_ARC% = 0%
		GOTO 790
...
760		!
		...
		SMG_WINDOW::FLAGS = (SMG_WINDOW::FLAGS OR 2%)
		PO_REG_SUB_LINE.READONLY_ARC% = -1%
...
770		!
		...
		!
		CALL ASSG_FREECHANNEL(PO_REG_SUB_LINE.CH_ARC%)

		EXIT FUNCTION

790		SMG_WINDOW::CHAN  = PO_REG_SUB_LINE.CH_ARC%
		WHEN ERROR IN
			RESET #PO_REG_SUB_LINE.CH_ARC%
			GET #PO_REG_SUB_LINE.CH_ARC%, REGARDLESS
		USE
			CONTINUE 32767
		END WHEN
