1	%TITLE "Maintain Purchase Order Register"
	%SBTTL "PO_MAIN_REGLINE"
	%IDENT "V3.6a Calico"

	FUNCTION LONG PO_MAIN_REGLINE(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	!
	!	.p
	!	The ^*Maintain Purchase Order Register\* option provides the
	!	means to enter and maintain
	!
	! Index:
	!
	!
	...
	! Compile:
	!
	!	$ BAS PO_SOURCE:PO_MAIN_REGLINE/LINE
	...
	!		Modified view to properly display PO number with the
	!		dash inserted in it.
	!
	...
	!
	!	10/06/98 - Kevin Handy
	!		Modifications to allo for limiting view to a
	!		single Vendor/PO
	!
 ...
 E0Loop:	SCOPE::SCOPE_EXIT = 0%
	...
	!--
			...
			CASE SMG$K_TRM_F14
				...
				THEN
					PO_REG_LINE::PO_TYPE = PO_TYPE::POTYPE
				END IF
				GOTO E0Loop
...
20300	CASE OPT_TESTENTRY
	...
	CASE OPT_DISPLAY
		...
		THEN
			...
			THEN
				UTL_LOCATION::LOCNAME  = &
					STRING$(LEN(UTL_LOCATION::LOCNAME), &
					A"?"B)
			END IF
		...
		THEN
			...
			THEN
				UTL_MEASURE::DESCRIPTION  = &
					STRING$(LEN(UTL_MEASURE::DESCRIPTION), &
					A"?"B)
			END IF
