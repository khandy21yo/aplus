1	%TITLE "Maintain Purchase Order Register"
	%SBTTL "PO_MAIN_REGLINE_01"
	%IDENT "V3.6a Calico"

	FUNCTION LONG PO_MAIN_REGLINE_01(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	!	maintains the Purchase Order Archive file.
	!
	! Index:
	!
	!
	...
	! Compile:
	!
	!	$ BAS PO_SOURCE:PO_MAIN_REGLINE_01/LINE
	...
	!		Based on PO_MAIN_REG_LINE, with changes to the
	!		view format.
	!
	...
	CASE OPT_INIT
		...
		SMG_WINDOW::NHELP  = "PO_MAIN_REGLINE_01"
		SMG_WINDOW::HSIZE  = 76%
		SMG_WINDOW::VSIZE  = 15%
		SMG_WINDOW::HVIEW  = 78%
		SMG_WINDOW::VVIEW  = 15%
...
750		WHEN ERROR IN
		...
		USE
			...
			PO_MAIN_REGLINE_01 = ERR
			CONTINUE 770
		END WHEN

		PO_REG_LINE.READONLY% = 0%
		GOTO 790
...
790		SMG_WINDOW::CHAN  = PO_REG_LINE.CH%
		WHEN ERROR IN
...
20200	!********************************************************************
 ...
 E0Loop:	SCOPE::SCOPE_EXIT = 0%
	...
	!	.p
	!	The ^*Purchase Order Number\* field is provided to enter the purchase
	...
	!	^*(04) Vendor Number\*
	!	.p
	!	The ^*Vendor Number\* field is provided to enter the
	...
	!	^*(05) Location\*
	!	.p
	!	The ^*Location\* field is provided to specify which of the
	...
	!--
			...
			THEN
				...
				THEN
					PO_REG_LINE::FROMLOCATION = UTL_LOCATION::LOCATION
				END IF
				GOTO E0Loop
			END IF
	...
	!--
			...
			THEN
				...
				THEN
					PO_REG_LINE::PRODUCT = PD_PRODUCT::PRODUCT_NUM
				END IF
				GOTO E0Loop
			END IF
	...
	!	^*(09) Order Date\*
	!	.p
	!	The ^*Order Date\* field is provided to enter the date on which the Purchase
	!	Order is effective.
	!	.p
	...
	!	.p
	!	The ^*Open__Closed Flag\* field is used to indicate
...
20300	CASE OPT_TESTENTRY
		...
		CASE 3%
			...
			!
			PO_MAIN_REGLINE_01 = FUNC_TESTENTRY(SMG_WINDOW, &
		...
		CASE 4%
			...
			! Is the input defined?
			!
			ST%, PO_MAIN_REGLINE_01 = FUNC_TESTENTRY(SMG_WINDOW, &
		...
		CASE 5%
			...
			!
			PO_MAIN_REGLINE_01 = FUNC_TESTENTRY(SMG_WINDOW, &
				PO_REG_LINE::FROMLOCATION, UTL_LOCATION::LOCNAME, &
		...
		CASE 6%
			...
			THEN
				...
				PO_REG_LINE::DESCRIPTION = PD_PRODUCT::DESCRIPTION
...
20500	CASE OPT_SETOLD
	...
	CASE OPT_VIEW
		...
		CASE 1%
			MVALUE = "  PO Number   Line Typ Vendor     Loc  " + &
				"Product #      UM O/C Description"

		!
		! Positions of lines
		!
		CASE 2%
			MVALUE = "014,019,023,034,039,054,057,061"
		...
		CASE 3%
			...
			IF LEN(MVALUE) > 3%
			THEN
				TEMP1$ = MID(MVALUE, 3%, LEN(PO_REG_LINE::VENDOR))
				TEMP2$ = RIGHT(MVALUE, 3% + LEN(PO_REG_LINE::VENDOR))
				IF TEMP1$ <> PO_REG_LINE::VENDOR
				THEN
					PO_MAIN_REGLINE_01 = 2%
					EXIT FUNCTION
				END IF
				IF TEMP2$ <> "" AND TEMP2$ <> PO_REG_LINE::PO
				THEN
					PO_MAIN_REGLINE_01 = 2%
					EXIT FUNCTION
				END IF
	...
	CASE OPT_FIND
		...
		CASE 0%
			FIND #PO_REG_LINE.CH%, KEY #0% GE PO_REG_LINE::PO + &
