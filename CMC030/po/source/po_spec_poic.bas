1	%TITLE "Print Received Order Journal"
	...
	!	.b
	!	.lm +5
	!	Special program to generate a transaction file to adjust
	!	the inventory "On Order" quantities so that they become
	!	in balance win the PO and the WP systems.
	!	.lm -5
	!
	! Index:
	!
	...
	! Compile:
	!
	!	$ BAS PO_SOURCE:PO_SPEC_POIC/LINE
...
690	!
	! Creat a Journal to hold our new records
	!
	BATCH_NO$ = "XX"
...
17004	WHEN ERROR IN
	...
	CASE "C"
		...
		GOTO ExitFunction IF (PD_PRODUCT::CATEGORY > TO_ITEM$) AND &
			TO_ITEM$ <> ""

	CASE "D"
		GOTO ExitFunction IF (PD_PRODUCT::DESCRIPTION > TO_ITEM$) AND &
			TO_ITEM$ <> ""

	CASE "P"
		GOTO ExitFunction IF (PD_PRODUCT::PRODUCT_NUM > TO_ITEM$) AND &
			TO_ITEM$ <> ""

	CASE "T"
		GOTO ExitFunction IF (PD_PRODUCT::PROD_TYPE> TO_ITEM$) AND &
			TO_ITEM$ <> ""
...
17100	WHEN ERROR IN
		FIND #PO_REG_SUB_LINE.CH%, KEY #0% EQ PO_REG_LINE::PO + &
