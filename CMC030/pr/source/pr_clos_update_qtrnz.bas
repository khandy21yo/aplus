1	%TITLE "Payroll Update Process"
	%SBTTL "PR_CLOS_UPDATE_QTRNZ"
	%IDENT "V3.6a Calico"
	...
	!	quarter.
	!	.b
	!	^*Note:\* A record in a payroll file folder ^~cannot\~ be edited after the file folder
	!	has been updated.
	!	.lm -5
	!
	! Index:
	...
	!
	! Option:
	!	PR_CLOS_UPDATE_QTRNZ$CONFIRM
	!
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_CLOS_UPDATE_QTRNZ/LINE
	!	$ LINK/EXECUTABLE=PR_EXE: PR_CLOS_UPDATE_QTRNZ, FUNC_LIB:CMCLINK/OPTION
	...
	!		Taken from PR_CLOS_UPDATE_QTR to create new
	!		version PR_CLOS_UPDATE_QTRNZ which doesn't zero
	!		out totals before updating.  This is for those
	!		who have folders spread through several accounts.
	!
	...
	!		Deleted commedted out lines so that they will
	!		stop showing up on searches.
	!
	...
	!		and never used.  Cleaned out unnecessary line numbers
	!		(8040,8045,8050). Fixed error trapping for control
	!		file from 310 to 320.
	!
	!	01/04/91 - Kevin Handy
	!		Found no changes necessary for ACCRUAL.
	!
	...
	! Abstract:FLD02
	!	.x To>Folder Date
	!	^*To Payroll Folder Date\*
	!	.b
	!	.lm +5
	!	The ^*Payroll Folder Date\* field
	!	selects the last folder date to update.
	!	.b
	!	The format for entry is MMDDYYYY or MMDDYY.
	!	.lm -5
	!
	...
	!	.b
	!	.lm +5
	!	The ^*Quarter\* field selects the accounting quarter
	!	to update.
	!	.b
	!	Valid entries are:
	!	.lm +10
...
1080	PR_HIS_PAY::UPDATE_FLAG = PR_HIS_PAY::UPDATE_FLAG OR 1%
...
2090	PR_HIS_DED::UPDATE_FLAG = PR_HIS_DED::UPDATE_FLAG OR 1%
