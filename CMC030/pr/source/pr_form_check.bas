1	%TITLE "CHECK - Print Payroll Checks"
	...
	!		Modified to print excess on it's own line,
	!		improved handling of the three columns.
	!
	!	01/14/91 - Craig Tanner
	!		Added YYYY$ to some filenames in error trapping.
	!
	!	03/18/91 - Kevin Handy
	!		Added eval_date field to read_rate.
	!
	!	05/17/91 - Kevin Handy
	!		Increased size of Pieces field by one by
	!		decreasing the size of the description.
	!
	...
	!		Added code to handle "A" (accrual) pay types in
	!		REG_ERNDED. (Ignore them)
	!
	!	12/18/91 - Kevin Handy
	!		Modified to ignore "A" types from PR_PAY files.
	!
	!	06/09/92 - Kevin Handy
	!		Lost BREAK_FLAG code, which didn't do anything
	!		useful. (It may have at one time)
	!
	!	06/16/92 - Kevin Handy
	!		Modified to use OUTP_INITFORM function.
	!
	!	01/22/93 - Kevin Handy
	!		Modifications to be able to handle checks with
	!		hardwired blanks when <<FRM-LENGTH>> is zero.
	!
	!	04/28/93 - Kevin Handy
	!		Fixed bug where was "SELECT PAYCODE$" where should
	!		have been "SELECT VAR1$" in several places in LOADVAR.
	!
	!	06/09/93 - Kevin Handy
	!		Don't print zero checks.
	!
	...
	!		Lost deleting display SMG_BLANK, which is never
	!		created.
	!
	!	05/09/95 - Kevin Handy
	!		Added field to allow printing zero/negative
	!		checks.
	!
	...
	!		Handle 'F' Future Deduction code
	!
	...
	!		Zero out NON_TOTAL amounts.
	!
	!	12/15/99 - Kevin Handy
	!		Improve number of digits displayed in summary wherever
	...
	!		Use WHEN ERROR IN
	!
	!	09/09/2002 - Kevin Handy
	!		Add accrual listings to check stub for new accruals
	!
	!	07/21/2005 - Kevin Handy
	!		Modified lookup of default pay rate to try to find
	!		code from employee master file first, then anything
	!		else after that fails.
	!
	!	11/15/2005 - Kevin Handy
	!		Make check number an integer instead of a float.OP
	!--
	%PAGE
...
18300	!********************************************************************
	...
	TEXT1$(TEXT_LOOP%) = &
		"TOTAL" + SPACE$(17%) + &
		FORMAT$(EMP_SUB_PAY::REG_HR, "###.##") + &
		FORMAT$(EMP_SUB_PAY::OVT_HR, "###.##") + &
		FORMAT$(EMP_SUB_PAY::PIECE, "#####.#") + &
	...
	THEN
		...
		FOR I% = 1% TO NON_LOOP%
			...
			TEXT$(TEXT_LOOP%) = LEFT(CODE$ + "   ", 3%) + &
				LEFT(DESCR$ + SPACE$(20%), 20%) + &
				SPACE$(19%) + &
				FORMAT$(EMP_TOTAL_NON(I%)::AMOUNT, "######.##") + &
				FORMAT$(EMP_TOTAL_NON(I%)::YTD_AMOUNT, "#######.##")
	...
	!
	! NOTE: The IF statement is probibly unecessary, but it makes it
	! clearer that the lines are not used when the number is zero.
	!
	IF FORM_GROUP(FRM_LENGTH%)::NUMBER <> 0%
	THEN
