1	%TITLE "MAINT - Maintain Payroll Journal"
	...
	!	.b
	!	.lm +5
	!	The ^*Maintain Payroll Journal\* option
	!	accesses the records in a specified payroll folder file.
	!	.b
	...
	!
	! Option:
	!	PR_MAIN_DETAIL_EMP_QUERY$HELP
	!	PR_MAIN_TRN_PAY$HELP
	!	PR_MAIN_TRN_DED$HELP
	!	PR_MAIN_TRN_CHECK$HELP
	!
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_JOUR_DETAIL/LINE
...
300	!******************************************************************
	...
	SMG_STATUS% = SMG$PASTE_VIRTUAL_DISPLAY(SMG_SCREEN_DATA%, SCOPE::SMG_PBID, 1%, &
		1%)
...
320	!
	...
	PR_TRN_PAY_DATE$ = ENTR_3DATE(SCOPE,  SMG_SCREEN_DATA%, "", &
		"Enter Payroll Folder Date (MMDDYYYY) ", BATCH_NO$, &
		64%, "8", "")
...
400	!*********************************************************************
	...
	THEN
		CALL ENTR_3MESSAGE(SCOPE, &
			"NOTE: Folder has been closed, no changes allowed", 0%)
		CLOSE_FLAG% = -1%
		GOTO 500
	END IF
	...
	THEN
		CALL ENTR_3MESSAGE(SCOPE, &
			"NOTE: Folder has been posted, no changes allowed", 0%)
		CLOSE_FLAG% = -1%
	END IF
...
500	!
	...
	V% = MAIN_WINDOW(PR_MAIN_DETAIL_EMP_QUERY.ID, "")
...
20000	FUNCTION LONG MAINT_GROUP(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	EXTERNAL LONG FUNCTION PR_MAIN_DETAIL_EMP_QUERY
	EXTERNAL LONG FUNCTION PR_MAIN_TRN_PAY
	...
	!
	CASE PR_MAIN_DETAIL_EMP_QUERY.ID

		MAINT_GROUP = PR_MAIN_DETAIL_EMP_QUERY(SMG_WINDOW, &
			MOPTION, MLOOP, MFLAG, MVALUE)

	!
	! Pay File
	!
	CASE PR_MAIN_TRN_PAY.ID

		MAINT_GROUP = PR_MAIN_TRN_PAY(SMG_WINDOW, &
			MOPTION, MLOOP, MFLAG, MVALUE)
