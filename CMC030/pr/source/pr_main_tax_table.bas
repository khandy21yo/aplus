1	%TITLE "Payroll Tax Table Maintenance"
	...
	!	accesses tax table files for Federal,
	...
	!		Allow editing all fields. Getting to be too
	!		much of a pain when setting up other taxes.
	!
	!	05/12/97 - Kevin Handy
	!		Use one mode digit for fica rates.
	!
	!	08/25/97 - Kevin Handy
	!		Change 'val' to 'val%'
	!
	!	07/10/98 - Kevin Handy
	!		Force fields 1-3 to be upper case
	!
	...
	!		Drop "THRESHOLD" field, and replace it with
	!		the new 'ROUNDING' field
	!		Use WHEN ERROR IN
	!
	...
	!		Replaced Exemption Threshold with Mimimum Tax.
	!--
	%PAGE
	...
	CASE OPT_INIT
		...
		! Define window
		!
		SMG_WINDOW::DESCR = "Payroll Tax Table Maintenance " + YYYY$
...
20200	CASE OPT_ENTRY
 ...
 E0Loop:	SCOPE::SCOPE_EXIT = 0%
	...
	!
	!--
			PR_TAX_TABLE::AUTH = EDIT$(ENTR_3STRINGLIST(SCOPE, &
				SMG_WINDOW::WNUMBER, "1;37", TEMP$, &
				PR_TAX_TABLE::AUTH, MFLAG OR 16%, "'", MVALUE, &
				ST$(), STTITLE$, "005"), -1%)
	...
	!	.p
	!	The ^*Status\* field
	!	records the marital status code for a specific table.
	!	.p
	!	Standard codes are:
	!	.b
	!	.lm +5
	...
	!--
			...
			PR_TAX_TABLE::TSTATUS = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, "3;37", TEMP$, &
				PR_TAX_TABLE::TSTATUS, MFLAG OR 16%, &
				"'", MVALUE)
	...
	!	^*(04) OASDI Employer %\*
	!	.p
	!	The ^*OASDI Employer %\* field
	!	enters the employer's OASDI portion of the
	...
	!--
			...
			PR_TAX_TABLE::FICA_EMPR_PCT = FUNC_ROUND( &
				...
				"7;29", TEMP$, &
				PR_TAX_TABLE::FICA_EMPR_PCT / 1000.0, TEMP%, &
				"##.###%", MVALUE) * 1000.0, 2%)
	...
	!	^*(05) OASDI Employee %\*
	!	.p
	!	The ^*OASDI Employee %\* field
	!	enters the employees' OASDI portion of the
	!	FICA rate expressed as a percentage,
	!	i.e., a rate of 6.20% must be entered as "6.20" rather than ".0620".
	!
	! Index:
	...
	!--
			...
			PR_TAX_TABLE::FICA_EMPE_PCT = FUNC_ROUND( &
				ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
				"8;29", TEMP$, &
				PR_TAX_TABLE::FICA_EMPE_PCT / 1000.0, TEMP%, &
				"##.###%", MVALUE) * 1000.0, 2%)
	...
	!	^*(06) OASDI Limit\*
	!	.p
	!	The ^*OASDI Limit\* field
	!	enters the dollar amount of an employee's earnings after which
	...
	!--
			...
			PR_TAX_TABLE::FICA_LIMIT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"9;25", TEMP$, &
	...
	!	^*(07) HI Employer %\*
	!	.p
	!	The ^*HI Employer %\* field
	!	enters the employer's HI portion of the
	!	FICA rate expressed as a percentage,
	...
	!--
			...
			PR_TAX_TABLE::FICA_EMPR_PCT_HI = FUNC_ROUND( &
				ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
				"11;29", TEMP$, &
				PR_TAX_TABLE::FICA_EMPR_PCT_HI / 1000.0, &
				TEMP%, "##.###%", MVALUE) * 1000.0, 2%)
	...
	!	^*(08) HI Employee %\*
	!	.p
	!	The ^*HI Employee %\* field
	!	enters the employees' HI portion of the
	!	FICA rate expressed as a percentage,
	!	i.e., a rate of 1.45% must be entered as "1.45" rather than ".0145".
	!
	! Index:
	...
	!--
			...
			PR_TAX_TABLE::FICA_EMPE_PCT_HI = FUNC_ROUND( &
				ENTR_3NUMBER(SCOPE, SMG_WINDOW::WNUMBER, &
				"12;29", TEMP$, &
				PR_TAX_TABLE::FICA_EMPE_PCT_HI / 1000.0, &
				TEMP%, "##.###%", MVALUE) * 1000.0, 2%)
	...
	!--
			...
			PR_TAX_TABLE::FICA_LIMIT_HI = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"13;25", TEMP$, &
	...
	!--
			...
			PR_TAX_TABLE::OT_ANL_MIN = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"7;57", TEMP$, &
	...
	!--
			...
			PR_TAX_TABLE::OT_ANL_PCT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"8;60", TEMP$, &
	...
	!--
			...
			PR_TAX_TABLE::OT_ANL_MAX = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"9;57", TEMP$, &
	...
	!	.p
	!	The ^*OST Deduction Limit\* field,
	!	for applicable jurisdictions, enters the dollar maximum
	...
	!--
			...
			PR_TAX_TABLE::OT_DED_MAX = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"10;57", TEMP$, &
	...
	!	.p
	!	The ^*SUI (State Unemployment Insurance) Minimum Limit\* field,
	!	for applicable jurisdictions, enters a dollar
	...
	!--
			...
			PR_TAX_TABLE::SUI_MIN = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"11;57", TEMP$, &
	...
	!	^*(15) SUI Percent\*
	!	.p
	!	The ^*SUI (State Unemployment Insurance) Percent\* field
	!	enters, in jurisdictions where applicable,
	...
	!--
			...
			PR_TAX_TABLE::SUI_PCT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"12;60", TEMP$, &
	...
	!	.p
	!	The ^*SUI (State Unemployment Insurance) Maximum Limit\* field
	!	enters, in jurisdictions where applicable, the
	...
	!--
			...
			PR_TAX_TABLE::SUI_MAX = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, &
				"13;57", TEMP$, &
	...
	!	.p
	!	The ^*Calculation Basis\* field
	!	enters, when applicable, an ^*F\*.
	!	.p
	...
	!	.p
	!	The ^*Basis %\* field
	!	enters a percentage amount when a tax jurisdiction requires that State
	...
	!--
			...
			PR_TAX_TABLE::BASIS_PCT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"2;18", TEMP$, &
	...
	!	.p
	!	The ^*Standard Withholding Allowance\* field
	!	enters the dollar amount of the withholding
	...
	!--
			...
			PR_TAX_TABLE::STD_WH = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"3;18", TEMP$, &
	...
	!	.p
	!	The ^*Adjusted Gross Percent\* field enters additional
	...
	!--
			...
			PR_TAX_TABLE::ADJ_GRS_PCT = ENTR_3NUMBER(SCOPE, &
	...
	!	.p
	!	The ^*Minimum Standard Adjustment\* field
	!	enters, in jurisdictions where applicable,
	...
	!--
			...
			PR_TAX_TABLE::MIN_STD_ADJ = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"5;18", TEMP$, &
	...
	!	.p
	!	The ^*Maximum Standard Adjustment\* field
	!	enters, in jurisdictions where applicable, the
	...
	!--
			...
			PR_TAX_TABLE::MAX_STD_ADJ = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"6;18", TEMP$, &
	...
	!	.p
	!	The ^*Personal Exemption\* field
	!	enters the dollar amount allowed for a personal exemption
	...
	!--
			...
			PR_TAX_TABLE::PR_EX = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"7;18", TEMP$, &
	...
	!	.p
	!	The ^*Additional Personal Exemption\* field
	!	enters the dollar amount allowed for additional personal exemptions
	!	according to current jurisdictional regulations. During the execution
	!	of the tax calculation process, the system will multiply the value of
	...
	!--
			...
			PR_TAX_TABLE::PR_EX_ADD = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"8;18", TEMP$, &
	...
	!	.p
	!	The ^*Federal Credit Percent\* field enters
	...
	!--
			...
			PR_TAX_TABLE::FED_CREDIT_PCT = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"9;18", TEMP$, &
	...
	!	^*(26) Rounding\*
	!	.p
	!	This field specifies what type of rounding should be done on
	!	the calculated tax.
	!	.b
	!	The options are:
	!	.b
	!	.list 0,"."
	!	.le
	!	*U Always round UP to the nearest dollar.
	!	.le
	!	*D Always round down to the nearest dollar.
	!	.le
	!	*N Round to the nearest dollar.
	!	.le
	!	<Other> Round to the penny.
	!	.els
	!
	! Index:
	!	.x Tax Table>Rounding
	!	.x Rounding>Tax Table
	!
	!--

			PR_TAX_TABLE::DROUNDING = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::LWINDOW, "10;18", TEMP$, &
				PR_TAX_TABLE::DROUNDING, MFLAG, &
				"'E", MVALUE)
	...
	!	.p
	!	The ^*Minimum Tax\* fiels is used to handle Arizona's
	!	tax idea that everyone is required to pay a minimum
	!	amount of tax every year, no matter what their income
	!	is.
	!
	! Index:
	!	.x Minimum Tax>Tax Table
	!	.x Tax Table>Minimum Tax
	!
	!--

			PR_TAX_TABLE::MINTAX = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"11;18", TEMP$, &
				PR_TAX_TABLE::MINTAX, MFLAG, &
				"#######.##", MVALUE)
	...
	!	.p
	!	The ^*Low Income Exclusion\* field
	!	enters the dollar amount below which the employee will not
	...
	!--
			...
			PR_TAX_TABLE::LOW_INCOME = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				"12;18", TEMP$, &
	...
	!	.p
	!	The ^*Over\* column
	!	enters the least whole dollar amount in a tax bracket. The tax
	...
	!--
			...
			PR_TAX_TABLE::OVER(TEMP%) = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				NUM1$(TEMP% + 3%) + ";36", TEMP$, &
	...
	!	.p
	!	The ^*Base\* column
	!	enters an annualized base tax amount which relates to
	...
	!--
			...
			PR_TAX_TABLE::TAXAMT(TEMP%) = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				NUM1$(TEMP% + 3%) + ";53", TEMP$, &
	...
	!	.p
	!	The ^*Percent\* column
	!	enters an amount expressed as a percentage. The percentage
	...
	!--
			...
			PR_TAX_TABLE::PLUS(TEMP%) = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::LWINDOW, &
				NUM1$(TEMP% + 3%) + ";69", TEMP$, &
...
27000	!*******************************************************************
	...
	INP$ = ENTR_3YESNO(SCOPE, SMG_WINDOW::WNUMBER, "", &
		"Confirm Copying Tax Table from " + &
		WORK_YYYY$ + " then press <Do>", &
		"N", 0%, "", "")
...
27010	CALL ASSG_CHANNEL(PR_TAX_TABLE.CH%, STAT%)
	...
	USE
		CALL ENTR_3MESSAGE(SCOPE, &
			"Prior year tax table does not exist", 0%)
		CONTINUE 27250
	END WHEN
...
27020	CALL ENTR_3MESSAGE(SCOPE, &
		"Creating new Tax Table file for " + YYYY$, 1%)

	CLOSE PR_TAX_TABLE.CH%

	SMG_STATUS% = LIB$DELETE_FILE(PR_TAX_TABLE.DEV$ + &
		"PR_TAX_TABLE_" + YYYY$ + ".TBL;*")
