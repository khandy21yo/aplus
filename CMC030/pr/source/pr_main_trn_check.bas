1	%TITLE "PR Check Journal Maintenance"
	%SBTTL "PR_MAIN_TRN_CHECK"
	%IDENT "V3.6a Calico"

	FUNCTION LONG PR_MAIN_TRN_CHECK(CDD_WINDOW_CDD SMG_WINDOW, &
	...
	! Abstract:HELP
	!	.p
	!	The ^*Check Journal\* is where data relative to manually
	!	prepared payroll checks can be entered. A record for an employee will preclude
	!	a check being written for the employee during the check printing routine.
	!
	! Index:
	!	.x Functions>Check
	!	.x Check>Function
	...
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_MAIN_TRN_CHECK/LINE
	...
	MAP (PR_TRN_CHECK_OLD) PR_TRN_CHECK_CDD PR_TRN_CHECK_OLD, &
		PR_TRN_CHECK2
	...
	MAP (TT_PR_TRN_CHECK) RARRAY_RECORD RARRAY(300%)
	...
	CASE OPT_INIT
		...
		SMG_WINDOW::NHELP = "PR_MAIN_TRN_CHECK"
		SMG_WINDOW::HSIZE = 78%
		SMG_WINDOW::VSIZE = 14%
		SMG_WINDOW::HPOS  = 2%
		SMG_WINDOW::VPOS  = 6%
...
20100	CASE OPT_BACKGROUND
		...
		SMG_STATUS% = SMG$PUT_CHARS(SMG_WINDOW::WNUMBER, &
			"  (01)         (02)       (03)               (04)" + &
			SPACE$(29%), 1%, 1%, , SMG$M_REVERSE)
		SMG_STATUS% = SMG$PUT_CHARS(SMG_WINDOW::WNUMBER, &
			"  End Date     Check  #   Check Date         " + &
			"Pay Freq" + SPACE$(25%), 2%, 1%, , SMG$M_REVERSE)
...
20200	CASE OPT_ENTRY
 ...
 E0Loop:	SCOPE::SCOPE_EXIT = 0%
	...
	!	^*(01) End Date\*
	!	.p
	!	The ^*End Date\* field indicates
	!	the Payroll End Date related to a specific payroll check.
	!
	! Index:
	!	.x End Date>Detail>Check
	!	.x Detail>Check>End Date
	!
	!--

			PR_TRN_CHECK::PR_END_DATE = BATCH_NO$ &
				IF PR_TRN_CHECK::PR_END_DATE = ""
			PR_TRN_CHECK::PR_END_DATE = ENTR_3DATE(SCOPE, &
				SMG_WINDOW::WNUMBER, XLINE$ + ";3", TEMP$, &
				PR_TRN_CHECK::PR_END_DATE, MFLAG, "8", MVALUE)
	...
	!	^*(02) Check _#\*
	!	.p
	!	The ^*Check _#\* field indicates
	!	the number of the specific check which was written payable to the
	!	subject employee.
	!
	! Index:
	!	.x Check Number>Detail>Check
	!	.x Detail>Check>Check Number
	!
	!--
	...
	!	^*(03) Check Date\*
	!	.p
	!	The ^*Check Date\* field indicates
	!	the date of the specific check which was written payable to the
	!	subject employee.
	!
	! Index:
	!	.x Check Date>Detail>Check
	!	.x Detail>Check>Check Date
	!
	!--
	...
	!	^*(04) Pay Frequency\*
	!	.p
	!	The ^*Pay Frequency\* field
	!	indicates the value of the Pay Frequency field in the subject
	!	employee's master file.
	!	.p
	!	The value of a Pay Frequency field must be equal to the number
	!	of regular payrolls in a calendar year.
	!
	! Index:
	!	.x Pay Frequency>Detail>Check
	!	.x Detail>Check>Pay Frequency
	!
	!--
			PR_TRN_CHECK::PAYFREQ = ENTR_3NUMBER(SCOPE, &
				SMG_WINDOW::WNUMBER, XLINE$ + ";48", TEMP$, &
				PR_TRN_CHECK::PAYFREQ * 1.0, MFLAG, &
				"###", MVALUE)
...
20300	CASE OPT_TESTENTRY
	...
	CASE OPT_TESTOPT
		...
		THEN
			...
			IF (PR_TRN_CHECK::UPDATE_FLAG AND 1%)
			THEN
				CALL HELP_3MESSAGE(SCOPE, &
					"PR Folder is Closed", "ERR", &
					"PR_CLOSED", "ERROR_PR_CLOSED")
				FUNC_FLAG% = 1%
			END IF

			IF (PR_TRN_CHECK::UPDATE_FLAG AND 2%) AND &
				FUNC_FLAG% = 0%
			THEN
				CALL HELP_3MESSAGE(SCOPE, &
					"PR Folder is Accrued", "ERR", &
					"PR_ACCRUED", "ERROR_PR_ACCRUED")
				FUNC_FLAG% = 1%
			END IF

			IF (PR_TRN_CHECK::UPDATE_FLAG AND 4%) AND &
				FUNC_FLAG% = 0%
			THEN
				CALL HELP_3MESSAGE(SCOPE, &
					"PR Folder has been Posted", "ERR", &
					"PR_POSTED", "ERROR_PR_POSTED")
				FUNC_FLAG% = 1%
			END IF
		END IF
...
27200			GET #SMG_WINDOW::CHAN, &
		...
		!
		CASE 6%
			PR_TRN_CHECK::EMPNUM = MID(MVALUE, 2%, 10%)
