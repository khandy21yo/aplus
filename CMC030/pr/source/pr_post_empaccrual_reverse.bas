1	%TITLE "Post Payroll Employee Accrual Program"
	%SBTTL "PR_POST_EMPACCRUAL_REVERSE"
	%IDENT "V3.6a Calico"
	...
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_POST_EMPACCRUAL_REVERSE
	!	$ LINK/EXECUTABLE=PR_EXE: PR_POST_EMPACCRUAL_REVERSE, FUNC_LIB:CMCLINK/OPTION
	!	$ DELETE PR_POST_EMPACCRUAL_REVERSE.OBJ;*
	!
	! Author:
	!
	!	12/21/93 - Kevin Handy
	!		Taken from PR_POST_EMPACCRUAL
	!
	! Modification history:
	!
	!	01/25/94 - Kevin Handy
	!		Disable posting to GL.
	!
	!	04/15/95 - Kevin Handy
	!		(V3.6)
	!		Update to V3.6 Calico coding standards
	!
	!	09/18/85 - Kevin Handy
 ...
 Init:	!==============================================================
	...
	!	^*(01) Post Period\*
	!	.p
	!	The ^*Post Period\* field
	!	enters the accounting period that the records are to be accrued.
	!	.p
	...
	!	designates any specific benefits
	...
	!	this field includes the codes for sick pay and personal leave
	...
	!	^*(05) Year End Date\*
	!	.p
	!	The ^*Year End Date\* field
	!	enters a date which determines
...
300	!
	...
	IF LEN(YYYY_PP$) <> 7%
	THEN
		CALL ENTR_3MESSAGE(SCOPE, LEFT(YYYY_PP$, 4%) + &
			RIGHT(YYYY_PP$, 6%) + &
	...
	THEN
		CALL ENTR_3MESSAGE(SCOPE, "The period must be within the range  of '01' to '" + &
...
3000	!
	...
	FOR PASS% = 1% TO 5%
		...
		CASE 2%
			...
			! Create transmittal
			!
			CALL ENTR_3MESSAGE(SCOPE, "Creating posting transmittals" + &
		...
		CASE 5%
			...
			IF EXIT_STATUS%
			THEN
				CALL HELP_3MESSAGE(SCOPE, "Fatal error! PARTIAL POSTING Done only!", &
...
3130		!
		...
		PR_ERNDED_DEF::DRCR_ACCT = STRING$(LEN(PR_ERNDED_DEF::DRCR_ACCT), 63%)
		PR_ERNDED_DEF::ACCRUAL_ACCT = STRING$(LEN(PR_ERNDED_DEF::ACCRUAL_ACCT), 63%)
...
3140		!
		...
		THEN
			...
			THEN
				CUR_HRS = PR_EMP_STD_ERNDED::LIMIT  - &
					(PR_EMP_STD_ERNDED::ACCRUED - PR_EMP_STD_ERNDED::CTDBAL)
			END IF
		END IF
		...
		THEN
			...
			! general ledger
			!
			IF COMP_STRING(EDIT$(PR_EMP_STD_ERNDED::CODE, -1%), CODE_NOT_POSTED$) = 0% &
...
3190		!
		...
		THEN
			...
			IF EDIT$(YEAR_END$, -1%) <> ""
			THEN
				IF MID(YEAR_END$, 5%, 2%) = MID(BATCH_NO$, 5%, 2%)
				THEN
					TEST% = -1%
				END IF
			ELSE
 ...
 AddAcct:
	...
	ELSE
		...
		THEN
			...
			IF GL_EXAM_CHART(ACCT_NUM$, GL_CHART_EXAM) <> CMC$_NORMAL
			THEN
				CALL ENTR_3MESSAGE(SCOPE, &
					"Employee " + TRM$(PR_EMP_MASTER::EMPNUM) + &
