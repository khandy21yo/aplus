1	%TITLE "Payroll Time Audit Report"
	...
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_RPRT_AUDT_TIME/LINE/NOOPT
	!	$ LINK/EXECUTABLE=PR_EXE: PR_RPRT_AUDT_TIME, FUNC_LIB:CMCLINK/OPTION
	...
	!
	!	10/13/89 - Frank F. Starman
	!		Fix bug (closing last channel) if there if more
	!		then OPEN_MAX periods
	!
	!	06/15/90 - Aaron Redd
	!		Added line layout information so that the report
	!		could be sent to a spreadsheet or a DIF file.
	!
	!	07/10/90 - Kevin Handy
	!		Split up error trapping.
	!
	!	12/18/91 - Kevin Handy
	!		Modified to ignore "A" records in PR_PAY.
	!
	!	02/02/94 - Kevin Handy
	!		(Temporary fix) increase OPEN_MAX to 50 to 80
	!		so DWI can get their report run.
	!
	!	02/02/94 - Kevin Handy
	!		Modified to use temporary file instead of trying
	!		to open up hundreds of files all at once.
	!
	!	02/10/94 - Kevin Handy
	!		Fixed FROM_ITEM which broke after 02/02/94 change.
	!
	!	02/10/94 - Kevin Handy
	!		Trap undefined employee error at 17120.
	!
	...
	!		(V3.6)
	!		Converted to V3.6 coding standards.
	!		Disabled unsolicited_input stuff.
	!		Fix call to PRNT_DATE that was missing "8%"
	!		parameter.
	!
	...
	!		Change 'val' to 'val%'
	!
	!	01/29/98 - Kevin Handy
	!		Added wildcard pay code.
	!
 ...
 Init:	!
	...
	!	.b
	!	.lm +5
	!	The ^*To Payroll Date\* field enter the date
	...
	!	item the report ends will end printing on.
	!	The value must be in agreement with
	!	field (05).
	!	.b
	...
	!	cause the report will be sorted by the indicated manner.
	!	.b
	!	Valid codes are:
	!	.table 3,25
	!	.te
	...
	!	.b
	!	.LM +5
	!	Specifies which pay codes will be included.
	!	All pay codes will be included if this fiels is blank.
	!	.lm -5
	!
	! Index:
	!
	...
	TOTALS_ONLY$ = LEFT$(UTL_REPORTX::OPTDEF(6%), 1%)

	!++
	! Abstract:FLD07
	!	.ts 55
	!	^*(07) Totals Only	YES, NO\*
	!	.b
	!	.LM +5
	!	Selects weither the report should contain detail information
	!	for each employee, or just a single total line.
	!	.lm -5
	!
	! Index:
	!
...
310	!
	...
	WHEN ERROR IN
		...
		OPEN "PR_TEMPORARY.TMP" FOR OUTPUT AS FILE PR_TEMPORARY.CH%, &
			...
			MAP PR_TEMPORARY, &
			PRIMARY KEY (PR_TEMPORARY::SORTBY, &
 ...
 ReportTitle:
	...
	TITLE$(5%) = SPACE$(70%) + &
		"--------------Hour-------------  -------Units-------"
	TITLE$(6%) = "Emp#  Payroll Date  Account            SubAcct    " + &
...
17125		!
		...
		CASE "DP"
			...
			SORTS$ = PR_EMP_MASTER::LOCATION + &
				PR_EMP_MASTER::DEPT + &
				PR_EMP_MASTER::EMPNUM

		CASE "LO"
			GOTO 17110 IF (PR_EMP_MASTER::LOCATION > TO_ITEM$) AND &
				TO_ITEM$ <> ""
			GOTO 17110 IF (PR_EMP_MASTER::LOCATION < FROM_ITEM$)
			SORTS$ = PR_EMP_MASTER::LOCATION + &
				PR_EMP_MASTER::EMPNUM
...
17350	!
 ...
 EmployeeTotal:
	...
	!
	IF (EMP_LINE_COUNTER% > 1%) OR &
		(TOTALS_ONLY$ = "Y" AND EMP_LINE_COUNTER% >= 1%)
	THEN
