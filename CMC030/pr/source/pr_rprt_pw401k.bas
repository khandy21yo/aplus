1	%TITLE "Payroll Audit Quarter Report"
	...
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_RPRT_PW401K/LINE
	...
	!
	!	07/14/98 - Kevin Handy
	!		Added code to handle the transmittal file.
	!
	...
	!		Code cleanup (check2)
	!
	!	07/06/99 - Kevin Handy
	!		Fix error trapping so last persion in file
	!		will get proper 401K dollars.
	!
	!	09/16/99 - Kevin Handy
	!		Fix program so that it doesn't skip opening
	!		deduction file when using unupdated folders
	!
	!	01/18/2000 - Kevin Handy
	!		Use "WHEN ERROR IN"
	!		Allow for the nonexistance of a deduction file
	!--
	%PAGE
 ...
 Init:	!
	...
	!	.p
	!	The ^*Start Payroll Date\* field contains the date of
	...
	!	^*(02) End Payroll Date\*
	!	.p
	!	The ^*End Payroll Date\* field contains the date of
...
18210	EMP_TOTAL(LOOP%) = FUNC_ROUND(EMP_TOTAL(LOOP%) + &
 ...
 PrintEmployeeTotal:
	...
	THEN
		...
		PRINT #K4FILE.CH%, &
			FORMAT$(SSN$, "'LLLLLLLLLL") + &
			FORMAT$(NAME1$, "'LLLLLLLLLLLLLLLLLL") + &
			FORMAT$(NAME2$, "'LLLLLLLLLLLLLLLLLLLL") + &
			RIGHT(PR_EMP_MASTER::BIRTH, 3%) + " " + &
			RIGHT(PR_EMP_MASTER::HIREDAY, 3%) + " " + &
			RIGHT(PR_EMP_MASTER::TERMDAY, 3%) + " " + &
			FORMAT$((EMP_TOTAL(1%) + EMP_TOTAL(2%)) * 100.0, "<0>####") + &
			FORMAT$(EMP_TOTAL(3%) * 100.0, "<0>##########") + &
			FORMAT$(EMP_TOTAL(4%) * 100.0, "<0>##########") + &
			"          "
	END IF
