1	%TITLE "Payroll Net Check Report"
	...
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_RPRT_TRN_CHECK/LINE
	!	$ LINK/EXECUTABLE=PR_EXE: PR_RPRT_TRN_CHECK, FUNC_LIB:CMCLINK/OPTION
	...
	!		Added ability to print Social Security Number
	!		on the-report. (Added field 05).
	!
	!	04/09/90 - Kevin Handy
	!		Added LYT_LINE$ information so we can output to a
	!		spreadsheet.
	!
	!	07/06/90 - Kevin Handy
	!		Modified so that it does not modify closed files
	!		by adding check records.
	!
	...
	!		Modified formats to allow one more digit in numbers
	!		without changing spacing. (Pulled off space from
	!		end of previous format statement)  King B had problem
	!		with units field when the negated out a large quanity.
	!
	...
	!		Use integer for #key
	!
	!	07/22/97 - Kevin Handy
	!		Display FICA OASDI and FICA HI as subtotals.
	!
	...
	DIM EMP_TOTAL(12%), GRAND_TOTAL(12%)
 ...
 Init:	!
	...
	!	^*(05) Print SSN\*
	!	.p
	!	The ^*Print SSN\* field specifies if you want
...
430	!
	...
	!
	! If journal net there the set journal map to history
	!
 !	IF USE_HISTORY%
 !	THEN
 !		PR_HIS_CHECK = PR_TRN_CHECK
 !	END IF
...
17020	!
	...
	!
	GOTO ExitTotal IF (PR_TRN_CHECK::CHECK > TO_ITEM$) AND TO_ITEM$ <> ""

	EMP_TOTAL(I%) = 0.0 FOR I% = 1% TO 12%
	...
	THEN
		...
		ELSE
			...
			CASE "FH"
				EMP_TOTAL(5%) = FUNC_ROUND(EMP_TOTAL(5%) + &
					PR_TRN_DED::AMOUNT, 2%)
				EMP_TOTAL(12%) = FUNC_ROUND(EMP_TOTAL(12%) + &
					PR_TRN_DED::AMOUNT, 2%)
...
17100	!
 ...
 ExitTotal:
	...
	TEXT$ = "                  Grand Total                    " + &
		FORMAT$(GRAND_TOTAL(11%), "Fica (OASDI) = ######.##") + &
		FORMAT$(GRAND_TOTAL(12%), "  Fica (HI) = ######.##") + &
		"  Employee count = " + FORMAT$(EMP_COUNT%, "#####")
