1	%TITLE "Payroll Register Report"
	...
	!	.b
	!	.lm +5
	!	The ^*Payroll Register\* report option prints a
	...
	! Compile:
	!
	!	$ BAS PR_SOURCE:PR_RPRT_TRN_REG/LINE
	!	$ LINK/EXECUTABLE=PR_EXE: PR_RPRT_TRN_REG, FUNC_LIB:CMCLINK/OPTION
	...
	!
	!	02/07/89 - Kevin Handy
	!		Added a grand total.
	!
	!	02/07/89 - Kevin Handy
	!		Fixed Sortby to handle location properly,
	!		instead of using 'LO' sometimes and 'DP' at
	!		other times in the select.
	!
	!	02/07/89 - Kevin Handy
	!		Added totals by location, if printing
	!		in location order.
	!
	!	02/08/89 - Kevin Handy
	!		Removed YTD amounts.  made it easier to summarise
	!		amounts by using records/arrays and calling
	!		subroutines do summarize/print data.
	!
	!	02/22/89 - Kevin Handy
	!		Continued format changes.
	!
	!	03/07/89 - Kevin Handy
	!		Fixed problem with overtime not being zeroed,
	!		and deductions summarizing into one item.
	!
	!	03/07/89 - Kevin Handy
	!		Added SU as a tax type.
	!
	!	03/27/89 - Kevin Handy
	!		Fixed problem where an employee with two checks in
	!		the file would be printed out three or four
	!		times on the report.
	!
	!	03/27/89 - Kevin Handy
	!		Fixed problem where overtime was not showing
	!		up in the net check amount.
	!
	!	04/03/89 - Kevin Handy
	!		Increased size of regular and overtime hours
	!		fields due to overflow on Coastal Group.
	!
	!	04/17/89 - Kevin Handy
	!		Increased size of Net field.
	!
	!	04/11/90 - Kevin Handy
	!		Added "Department" to printout.
	!
	!	04/24/90 - Kevin Handy
	!		Added Department sub-totals (only while printing
	!		in location order).
	!
	!	12/18/91 - Kevin Handy
	!		Modified to ignore "A" records in PR_PAY file.
	!
	!	01/16/95 - Kevin Handy
	!		Modified to print even if deduction/check folders
	!		do not exist.
	!
	!	03/07/95 - Kevin Handy
	!		Clean up (Check)
	!
	!	03/17/95 - Kevin Handy
	!		(V3.6)
	!		Fixed bug where it was limited to 10 lines of
	!		codes due to an undefined array EMP_DESCR$(,)
	...
	!		Double dimensions (from 30 and 100 to 60 and 200)
	!
	!	03/13/97 - Kevin Handy
	!		Handle FH Code.
	!
	!	05/15/97 - Kevin Handy
	!		Fix bug in handling "FH" code.
	!
	!	05/29/98 - Kevin Handy
	!		Handle new 'F' final deduction code
	!
	!	06/09/98 - Kevin Handy
	!		Added entry fields (05) and (06)
	!
	...
	!		Increase field size of several numbers so that they
	!		don't overflow as easily (PWELLS)
	!--
	%PAGE
