1	%TITLE "Sales by Customer and Product"
	...
	! Compile:
	!
	!	$ BAS SA_SOURCE:SA_RPRT_CUSPRDSAL3/LINE
 ...
 Initialization:
	...
	SECWLDCRD$ = EDIT$(UTL_REPORTX::OPTDEF(3%), -1%)
	...
	!	.b
	!	.lm +5
	!	The ^*Number of Years\* field is to be entered with the
	...
	FOR Y% = SY% TO YEARS%
		...
		THEN
			...
			GOTO GetHistRec IF COMP_STRING(EDIT$( &
				PD_PRODUCT_EXAM::SECONDARY_CODE, -1%), &
				SECWLDCRD$) = 0% &
				AND SECWLDCRD$ <> ""
		ELSE
			NO_TEST%(Y%) = 1%
...
17135	NEXT Y%
 ...
 PrintLine:
17500	GOTO ExitPrintLine IF TOTAL_ONLY$ = "Y"
	...
	FOR Y% = 1% TO YEARS%
		...
		IF ((Y% = 1%) OR (Y% > 1%) AND (PRINT_VARIANCE$ <> "Y"))
		THEN
			TEXT$ = "Units " + &
...
17590	FOR Y% = 1% TO YEARS%
 ...
 CustTotal:
 !	GOTO ExitCusTotal IF TOTAL_ONLY$ = "Y"
...
32767	!******************************************************************
	! End of report SA_RPRT_CUSPRDSAL3
	!******************************************************************
	END
