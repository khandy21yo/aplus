1	%TITLE "Sales by Product and Salesman"
	...
	! Compile:
	!
	!	$ BAS SA_SOURCE:SA_RPRT_PRDCUS/LINE
	...
	!		Remove reference to SA_SALESMAN_CDD, which was
	...
	DECLARE CUSTOMER_TOTAL_STR CUSTOMER_TOTAL(3000%)
	DECLARE CUSTOMER_TOTAL_STR NEW_CUSTOMER_TOTAL
 ...
 Initialization:
	...
	!	prints the number of the fiscal years. If
...
17120	SY% = Y%
 ...
 TestKey:
	...
	!
	! Calculate NO_TEST% and SALE.TEST%, whatever they may mean
	!
	NO_TEST% = 1%
	FOR Y% = 1% TO YEARS%
		IF NO_TEST%(Y%) = 0%
	...
	FOR Y% = 1% TO YEARS%
		...
		THEN
			...
			LAST.CROSSREF$ = IC_HISTORY(Y%)::CROSSREF
...
17135	NEXT Y%
 ...
 PrintLine:
	FOR Y% = 1% TO YEARS%
		...
		NEW_CUSTOMER_TOTAL::YEAR = YYYY$(Y%)
		NEW_CUSTOMER_TOTAL::CUSTOMER = LAST.CROSSREF$

		IF PRINT_UNITS$ = "Y" AND (Y% = 1% OR Y% > 1%)
		THEN
			NEW_CUSTOMER_TOTAL::XLINE = "3"
			NEW_CUSTOMER_TOTAL::INFO(QI%) = PQUANTITY(Y%, QI%) &
				FOR QI% = 1% TO 12%
			GOSUB SummarizeCust
		END IF

		NEW_CUSTOMER_TOTAL::XLINE = "6"
		NEW_CUSTOMER_TOTAL::INFO(QI%) = PRICEAMT(Y%, QI%) &
			FOR QI% = 1% TO 12%
		GOSUB SummarizeCust

		IF GROSS_PROFIT$ = "Y" AND (Y% = 1% OR Y% > 1%)
		THEN
			NEW_CUSTOMER_TOTAL::XLINE = "9"
			NEW_CUSTOMER_TOTAL::INFO(QI%) = &
				PRICEAMT(Y%, QI%) - COSTAMT(Y%, QI%) &
				FOR QI% = 1% TO 12%
	...
	FOR Y% = 1% TO YEARS%
		...
		FOR I% = 1% TO 12%
			SALEPRICEAMT(Y%, I%) = SALEPRICEAMT(Y%, I%) + &
				PRICEAMT(Y%, I%)
			SALECOSTAMT(Y%, I%) = SALECOSTAMT(Y%, I%) + COSTAMT(Y%, I%)
		NEXT I%
 ...
 SummarizeCust:
	...
	FOR ZI% = 1% TO CUSTOMER_TOTAL%
		GOTO SumFound &
			IF NEW_CUSTOMER_TOTAL::CUSTOMER = CUSTOMER_TOTAL(ZI%)::CUSTOMER AND &
			NEW_CUSTOMER_TOTAL::XLINE = CUSTOMER_TOTAL(ZI%)::XLINE AND &
			NEW_CUSTOMER_TOTAL::YEAR = CUSTOMER_TOTAL(ZI%)::YEAR
	NEXT ZI%

	CUSTOMER_TOTAL% = CUSTOMER_TOTAL% + 1%

	CUSTOMER_TOTAL(CUSTOMER_TOTAL%) = NEW_CUSTOMER_TOTAL

	RETURN

 SumFound:
	CUSTOMER_TOTAL(ZI%)::INFO(ZJ%) = &
		CUSTOMER_TOTAL(ZI%)::INFO(ZJ%) + &
		NEW_CUSTOMER_TOTAL::INFO(ZJ%) &
		FOR ZJ% = 1% TO 12%
 ...
 UstaPrintLine:
	...
	!
	! Sort the array of totals
	!
	FOR I% = 1% TO CUSTOMER_TOTAL%

		FOR J% = 1% TO CUSTOMER_TOTAL% - I%

			IF CUSTOMER_TOTAL(J%)::CUSTOMER + &
				CUSTOMER_TOTAL(J%)::YEAR + &
				CUSTOMER_TOTAL(J%)::XLINE > &
				CUSTOMER_TOTAL(J% + 1%)::CUSTOMER + &
				CUSTOMER_TOTAL(J% + 1%)::YEAR + &
				CUSTOMER_TOTAL(J% + 1%)::XLINE
			THEN
				NEW_CUSTOMER_TOTAL = CUSTOMER_TOTAL(J%)
				CUSTOMER_TOTAL(J%) = CUSTOMER_TOTAL(J% + 1%)
				CUSTOMER_TOTAL(J% + 1%) = NEW_CUSTOMER_TOTAL
			END IF
	...
	FOR I% = 1% TO CUSTOMER_TOTAL%
		...
		IF CUSTOMER_TOTAL(I%)::CUSTOMER <> LAST_CUST$
		THEN
			V% = AR_EXAM_CUSTOM(CUSTOMER_TOTAL(I%)::CUSTOMER, &
				AR_35CUSTOM_EXAM)

			CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), "", PAGE%)

			TEXT$ = "     " + &
				CUSTOMER_TOTAL(I%)::CUSTOMER + "     " + &
			...
			TEXT$ = "     " + &
				"               " + &
				AR_35CUSTOM_EXAM::ADD1

			CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), TEXT$, PAGE%)

			TEXT$ = "     " + &
				"               " + &
				AR_35CUSTOM_EXAM::ADD2

			CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), TEXT$, PAGE%) &
				UNLESS TEXT$ = ""

			TEXT$ = "                    " + &
				TRM$(AR_35CUSTOM_EXAM::CITY) + ", " + &
				TRM$(AR_35CUSTOM_EXAM::STATE) + "  " + &
				TRM$(AR_35CUSTOM_EXAM::ZIP) + " - " + &
				TRM$(AR_35CUSTOM_EXAM::PHONE)
		...
		IF CUSTOMER_TOTAL(I%)::YEAR <> LAST_YEAR$
		THEN
			TEXT$ = SPACE$(25%) + ":   "
			TEXT$ = "Year " + CUSTOMER_TOTAL(I%)::YEAR + &
				TEXT$ + TEXT$ + TEXT$ + TEXT$

			CALL OUTP_LINE("", UTL_REPORTX, TITLE$(), TEXT$, PAGE%)
		END IF

		LAST_CUST$ = CUSTOMER_TOTAL(I%)::CUSTOMER
		LAST_YEAR$ = CUSTOMER_TOTAL(I%)::YEAR
		PAGE% = 4%

		QTR(ZI%) = 0.0 FOR ZI% = 1% TO 4%
		QTR((ZI% + 2%) / 3%) = QTR((ZI% + 2%) / 3%) + &
			CUSTOMER_TOTAL(I%)::INFO(ZI%) FOR ZI% = 1% TO 12%

		SELECT CUSTOMER_TOTAL(I%)::XLINE

		CASE "3"
			TEXT$ = "Units " + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(1%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(2%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(3%), "#######") + &
				FORMAT$(QTR(1%), "########") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(4%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(5%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(6%), "#######") + &
				FORMAT$(QTR(2%), "########") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(7%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(8%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(9%), "#######") + &
				FORMAT$(QTR(3%), "########") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(10%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(11%), "#######") + &
				FORMAT$(CUSTOMER_TOTAL(I%)::INFO(12%), "#######") + &
				FORMAT$(QTR(4%), "########") + &
				FORMAT$(QTR(1%) + QTR(2%) + QTR(3%) + QTR(4%), "########")
