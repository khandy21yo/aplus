1	%TITLE "Sales Volume Report by Salesman"
	%SBTTL "SA_RPRT_SALVOLUME"
	%IDENT "V3.6a Calico"
	...
	! Abstract:HELP
	!	.p
	!	The ^*Sales Volume Report by Salesman\* option
	...
	! Compile:
	!
	!	$ BAS SA_SOURCE:SA_RPRT_SALVOLUME/LINE
	...
	!		Changed decimal value to allow higher numbers.
	!		Changed fields to allow higher numbers.
	!
...
330	!
 ...
 ReportTitle:
	...
	CASE "S"
		K_NUM% = 0%
		TITLE$(1%) = "SALES VOLUME BY SALESMAN NUMBER"

	CASE "C"
		K_NUM% = 2%
		TITLE$(1%) = "SALES VOLUME BY SALESMAN CLASS"

	CASE "T"
		K_NUM% = 1%
		TITLE$(1%) = "SALES VOLUME BY SALESMAN TYPE"
	...
	! Heading
	!
	TITLE$(6%) = "Salesman#  Name" + SPACE$(37%) + &
		"SType Class        Cost"       + &
...
17000	!******************************************************************
 ...
 GetNextRec:
	...
	IF PRINT_FLAG%
	THEN
		GOSUB BrokerTotal IF PRICE <> 0.0 OR COST <> 0.0
	END IF
...
17500	WHEN ERROR IN
 ...
 PrintTotals:
	...
	TEXT$ = "G R A N D   T O T A L" + SPACE$(43%) + &
		FORMAT$(TOTAL_COST,  "###,###,###") + " " + &
		FORMAT$(TOTAL_PRICE, "###,###,###") + " " + &
 ...
 BrokerTotal:
	...
	CASE "M"
		TEMP_SORTKEY = -FUNC_ROUND((PRICE - COST) / &
			PRICE * 100.0, 2%) IF PRICE <> 0.0
		TITLE$(2%) = "RANKED BY GROSS MARGIN"

	END SELECT

	TEMP_SALESMAN = SA_SALESMAN::SALESMAN
	TEMP_DESC = SA_SALESMAN::DESCR
	TEMP_TYPE = SA_SALESMAN::TTYPE
	TEMP_CLASS = SA_SALESMAN::CLASS
	TEMP_PRICE = PRICE
	TEMP_COST = COST
	...
	IF PRICE <> 0.0
	THEN
		TEMP_MARG = FUNC_ROUND((PRICE - COST) / PRICE * 100.0, 2%)
	ELSE
		TEMP_MARG = 0.0
	END IF
...
32767	!******************************************************************
	! End of report SA_RPRT_SALVOLUME
	!******************************************************************
	END
