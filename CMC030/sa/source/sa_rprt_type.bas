1	%TITLE "Job Type Report"
	...
	! Abstract:HELP
	!	.p
	!	The ^*Job Type Report\* option
	!	prints a list of the Job Type file.
	!
	! Index:
	!
	! Option:
	...
	! Compile:
	!
	!	$ BAS SA_SOURCE:SA_RPRT_TYPE/LINE
 ...
 Initialization:
	...
	!	printing to begin with the selected item.
	!	.p
	...
	!	report to end with the selected type.
	!	.p
...
300	WHEN ERROR IN
 ...
 ReportTitle:
	...
	! Title
	!
	TITLE$(1%) = "Subaccount Type Description"
...
17000	!******************************************************************
	...
	WHEN ERROR IN
		...
		THEN
			RESET #SA_TYPE.CH%
		ELSE
			FIND #SA_TYPE.CH%, KEY #0% GE FROM_ITEM$, REGARDLESS
		END IF
	USE
		FILENAME$ = "SA_TYPE"
		CONTINUE HelpError
...
17100	!******************************************************************
	...
	!
	GOTO ExitTotal IF (SA_TYPE::TTYPE > TO_ITEM$) AND TO_ITEM$ <> ""

	GOTO GetNextRec &
		IF COMP_STRING(EDIT$(SA_TYPE::TTYPE, -1%), WLDCRD$) = 0% &
		AND WLDCRD$ <> ""

	!
	! Print out one line
	!
	TEXT$ =  SA_TYPE::TTYPE	+ "     " + SA_TYPE::DESCR
