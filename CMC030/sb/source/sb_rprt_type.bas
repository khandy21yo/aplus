1	%TITLE "Sub Ledger Sub Account Type Report"
	%SBTTL "SB_RPRT_TYPE"
	%IDENT "V3.6a Calico"
	...
	! Compile:
	!
	!	$ BAS SB_SOURCE:SB_RPRT_TYPE/LINE
...
17000	!******************************************************************
	...
	WHEN ERROR IN
		...
		THEN
			RESET #SB_TYPE.CH%
		ELSE
			FIND #SB_TYPE.CH%, KEY #0% GE FROM_ITEM$, REGARDLESS
		END IF
	USE
		FILENAME$ = "SB_TYPE"
		CONTINUE HelpError
...
17100	!******************************************************************
	...
	!
	GOTO ExitTotal IF (SB_TYPE::TTYPE > TO_ITEM$) AND TO_ITEM$ <> ""

	GOTO GetNextRec &
		IF COMP_STRING(EDIT$(SB_TYPE::TTYPE, -1%), WLDCRD$) = 0% &
		AND WLDCRD$ <> ""

	!
	! Print out one line
	!
	TEXT$ =  SB_TYPE::TTYPE	+ "     " + SB_TYPE::DESCR
