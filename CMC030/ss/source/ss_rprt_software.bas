1	%TITLE "Support System Customer License Report"
	...
	!	. Software>License
	!	.p
	!	This report tells what version of the CMC software the
	...
	! Compile:
	!
	!	$ BAS SS_SOURCE:SS_RPRT_SOFTWARE/LINE
	!	$ LINK/EXE=SS_EXE: SS_RPRT_SOFTWARE, FUNC_LIB:CMCLINK/OPTION
	...
	DIM SS_CUS_SYSMENU_CDD LICENSE_LIST(100%)
...
17100	!******************************************************************
	...
	TEXT$ = AR_35CUSTOM::CUSNUM + " " + LEFT(AR_35CUSTOM::CUSNAM, 40%) + " " + &
		AR_35CUSTOM::ADD1 + "," + AR_35CUSTOM::ADD2 + " " + &
		AR_35CUSTOM::CITY + " " + AR_35CUSTOM::STATE + "    " + AR_35CUSTOM::ZIP
...
17350	WHEN ERROR IN
	...
	FOR I% = 1% TO LICENSE_LIST%

		IF (LICENSE_LIST(I%)::SYSTEM = SS_CUS_SYSMENU::SYSTEM) AND &
			(LICENSE_LIST(I%)::MENNUM = SS_CUS_SYSMENU::MENNUM)
		THEN
			LICENSE_LIST(I%)::INSDAT = SS_CUS_SYSMENU::INSDAT &
				IF LICENSE_LIST(I%)::INSDAT < SS_CUS_SYSMENU::INSDAT
			GOTO 17350
		END IF
	...
	LICENSE_LIST(I%)::SYSTEM = SS_CUS_SYSMENU::SYSTEM
	LICENSE_LIST(I%)::MENNUM = SS_CUS_SYSMENU::MENNUM
	LICENSE_LIST(I%)::INSDAT = SS_CUS_SYSMENU::INSDAT
	...
	!
17400	!
	! Sort the list (slowly)
	!
	FOR I1% = 1% TO LICENSE_LIST% - 1%

		FOR I2% = 1% TO LICENSE_LIST% - I1%

			IF LICENSE_LIST(I2%)::SYSTEM > &
				LICENSE_LIST(I2% + 1%)::SYSTEM
			THEN
				LICENSE_LIST(0%) = LICENSE_LIST(I2%)
				LICENSE_LIST(I2%) = LICENSE_LIST(I2% + 1%)
				LICENSE_LIST(I2% + 1%) = LICENSE_LIST(0%)
			END IF
	...
	FOR I% = 1% TO LICENSE_LIST%
		...
		TEXT$ = SPACE$(35%) + LICENSE_LIST(I%)::SYSTEM + "     " + &
			LICENSE_LIST(I%)::MENNUM + "  " + &
			PRNT_DATE(LICENSE_LIST(I%)::INSDAT, 0%)
...
19000	!******************************************************************
	...
	!******************************************************************
	! End of report SS_RPRT_SOFTWARE
	!******************************************************************
	END
