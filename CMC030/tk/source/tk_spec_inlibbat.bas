1	%TITLE "Create Batch File to Run TK_SPEC_INSRTLIB"
	...
	! Compile:
	!
	!	$ BAS TK_SOURCE:TK_SPEC_INLIBBAT/LINE
	...
	!
	!	5/23/91 - J. Shad Rydalch
	!		Added option for form fields to be extracted.
	!		changed Entry Type feild so that help message
	!		type was always correct, in case user key a
	!		list.
	!
	...
	SCOPE::PRG_PROGRAM = "TK_SPEC_INLIBBAT"
	SCOPE::PRG_ITEM = ""
	...
	SMG_STATUS% = SMG$PUT_CHARS(SMG_SCREEN_DATA%, &
		"EXTRACT HELP MESSAGES FROM MODULES", 2%, 20%)
 ...
 ComFile:
	...
	COM_FILE$ = EDIT$(ENTR_3STRING(SCOPE, SMG_SCREEN_DATA%, &
		"5;45", "Command file <TEMP.COM>", COM_FILE$, &
		16%, "'E", COM_FILE$), -1%)
...
100	!
	...
	!
	SMG_STATUS% = SMG$PUT_CHARS(SMG_SCREEN_DATA%, &
		"System name " + SPACE$(50%), 6%, 1%)

	SMG_STATUS% = SMG$PUT_CHARS(SMG_SCREEN_DATA%, &
		"Program, File or Menu (P,F,M,*)" + SPACE$(50%), 7%, 1%)
 ...
 Entrtyp:
	...
	OK_FLAG% = 0%
	!
	! Make sure there is at least one valid place to look
	!
	IF INSTR(1%, NAME$, "*") <> 0%
	THEN
		! do every thing
		...
		OK_FLAG% = -1%
	ELSE
		! look to see if to do other help messages
		!
		! Put together a Valid NAME$ from the info
		! user gave us, correct letters and commas
		!
		NAME1$ = ""
		NAME1$ = "P" IF INSTR(1%, NAME$, "P") <> 0%
		NAME1$ = "F" IF INSTR(1%, NAME$, "F") <> 0% AND &
			LEN(NAME1$) = 0%
		NAME1$ = NAME1$ + ",F" IF INSTR(1%, NAME$, "F") &
			<> 0% AND LEN(NAME1$) <> 0%
		NAME1$ = "M" IF INSTR(1%, NAME$, "M") <> 0% AND &
			LEN(NAME1$) = 0%
		NAME1$ = NAME1$ + ",M" IF INSTR(1%, NAME$, "M") &
			<> 0% AND LEN(NAME1$) <> 0%
		NAME$ = NAME1$
		OK_FLAG% = -1% IF LEN(NAME$) <> 0%
	END IF

	GOTO Entrtyp IF OK_FLAG% = 0%
 ...
 EntrMod:
	...
	MODULE$ = "*" IF INSTR(1%, NAME$, "M") <> 0% OR &
		INSTR(1%, NAME$, "*")

	MODULE$ = EDIT$(ENTR_3STRING(SCOPE, SMG_SCREEN_DATA%, &
		"8;45", "Module <Wildcard>", &
...
200	!
	...
	THEN
		...
		PRINT #COM_FILE.CH%, "$run tk_exe:tk_spec_insrtlib"
		PRINT #COM_FILE.CH%, DIRECT$
		PRINT #COM_FILE.CH%, '"' + TRM$(NAME$) + '"'
	...
	THEN
		PRINT #COM_FILE.CH%, '$write sys$output ""'
		PRINT #COM_FILE.CH%, "$run tk_exe:tk_spec_formfields"
		PRINT #COM_FILE.CH%, DIRECT$
		PRINT #COM_FILE.CH%, TRM$(MODULE$)
	END IF
