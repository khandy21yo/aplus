1	%TITLE "TK_SUBR_DOCUTPLATE Sub Process to Maintain a Function Template"
	%SBTTL "TK_SUBR_DOCUTPLATE"
	%IDENT "V3.6a Calico"

	SUB TK_SUBR_DOCUTPLATE
	...
	! ABSTRACT:HELP
	!	.p
	!	A process to aid in the maintenance of function templates.
	!
	! Index:
	!
	! Option:
	...
	! Compile:
	!
	!	$ BAS TK_SOURCE:TK_SUBR_DOCUTPLATE/LINE
	...
	SCOPE::PRG_PROGRAM = "TK_SUBR_DOCUTPLATE"
	SCOPE::PRG_ITEM = ""
...
50	!
	...
	FILE_NAME$ = "CMC:TK_FUNCTION_TYPE.TEMPLATE"
	PREFIX$ = "CMC:HLP_FUNCTION_TYPE_"
	SUFFIX$ = ".TEMPLATE"
...
300	!
	...
	SMG_STATUS% = SMG$LABEL_BORDER(SMG_SCROLL, " Function Template ", &
...
9000	!
	...
	!
	WHEN ERROR IN
		OPEN FILE_NAME$ FOR INPUT AS FILE #OLD.CH%, ACCESS READ, ALLOW MODIFY
	USE
		CONTINUE 9500
	END WHEN
...
10000	OPEN FILE_NAME$ FOR OUTPUT AS FILE #NEW.CH%
	...
	PRINT #NEW.CH%, EDIT$(NAME$(I%), 8% + 128%) + "<>." &
		FOR I% = WINDOW_SCROLL::BEG_ELEMENT TO (WINDOW_SCROLL::END_ELEMENT - 1%)
...
11000	!***************************************************************
	...
	CASE 1%
		...
		NAME$(ARRAY_LOOP%) = ENTR_3STRING(SCOPE, SMG_SCREEN_DATA%, "", FUN_NAME$ + &
			"name", SPACE$(55%), 32%, "'E", &
			NAME$(ARRAY_LOOP%))
 ...
 SaveFields:
	T_LINES$ = LINES$(ARRAY_LOOP%)
	T_NAME$ = NAME$(ARRAY_LOOP%)

	RETURN

 RestoreFields:
	LINES$(ARRAY_LOOP%) = T_LINES$
	NAME$(ARRAY_LOOP%) = T_NAME$
...
19000	!======================================================================
 ...
 HelpError:
	CALL ENTR_3MESSAGE(SCOPE, &
		"TK_SUBR_DOCUTPLATE-ERR:  " + NUM1$(ERR) + &
		" at line " + NUM1$(ERL), 0%)
