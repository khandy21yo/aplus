1	%TITLE "Print Requisition Form"
	...
	! Compile:
	!
	!	$ BAS WP_SOURCE:WP_FORM_REQJOUR/LINE
	...
	! Author:
	!
	!	07/25/91 - Craig Tanner
	!
	! Modification history:
	!
	!	07/29/91 - Craig Tanner
	!		Modifided to update Requisition line in journal.
	!
	!	08/31/92 - Kevin Handy
	!		Clean up (Check)
	!
	!	09/17/92 - Dan Perkins
	!		Added IC_BINMAP for binmap locations and
	!		IC_READ_35BALANCE for inventory quantities.
	!
	!	06/18/93 - Kevin Handy
	!		Clean up (Check)
	!
	!	02/16/95 - Kevin Handy
	!		Modified so from/to items can only be as long
	!		as a job number.
	!
	...
	!
	!	07/28/95 - Kevin Handy
	!		Modified to use OUTP_INITFORM instead of the
	!		hard coded logic that had been embeded into
	!		the source.
	!
	!	08/15/95 - Kevin Handy
	!		Added a "Regardless" to the get for the job
	!		description.
	!
	!	10/31/96 - Kevin Handy
	!		Clean up (Check)
	!
	!	03/31/97 - Kevin Handy
	!		Change FROM.ITEM to FROM_ITEM and TO.ITEM
	!		to TO_ITEM. Doesn't change code, but makes it
	!		more consistant.
	!		Lose setting values from TEMP.IDENT$, which was
	!		never initialized in the first place.
	!		Use TRM$ instead of EDIT$ one from/to entries.
	!
	!	09/09/97 - Kevin Handy
	!		Lose unecessary external definitions
	!
	!	04/03/1998 - Kevin Handy
	!		Fix some goofiness with the from/to items
	!
	...
	!		is never created
	!
	!	06/08/2000 - Kevin Handy
	!		Use WHEN ERROR IN
	!		Add several REGARDLESS clauses.
	!
	!	06/08/2000 - Kevin Handy
	!		Change to have FROM/TO requisition instead of
...
18040	WHEN ERROR IN
	...
	ELSE
		GOTO ReadLine &
			IF WP_REQLINE::REQNUM < REQUISITION_FROM$ OR &
...
18220	!
	...
	WHEN ERROR IN
		GET #IC_BINMAP.CH%, &
			KEY #0% EQ WP_REQLINE::PRODUCT + JC_JOB::LOCATION, &
			REGARDLESS
	USE
...
18310	WHEN ERROR IN
	...
	GOTO AssignReqNum &
		IF WP_REQREGISTER::REQNUM = WP_CONTROL::REQNUM
