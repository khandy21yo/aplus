1	%TITLE "Manufacturing WIP Order Buyoff Journal"
	%SBTTL "WP_MAIN_BUYOFF"
	%IDENT "V3.6a Calico"

	FUNCTION LONG WP_MAIN_BUYOFF(CDD_WINDOW_CDD SMG_WINDOW, LONG MOPTION, &
	...
	! Compile:
	!
	!	$ BAS WP_SOURCE:WP_MAIN_BUYOFF/LINE
	!	$ LIB/REP FUNC_LIB:CMCFUN WP_MAIN_BUYOFF
	!	$ DELETE WP_MAIN_BUYOFF.OBJ;*
	!
	! Author:
	!
	!	06/10/91 - Val James Allen
	!
	! Modification history:
	!
	!	03/17/92 - Dan Perkins
	!		Moved field descriptions to the left.
	!		Makes appearance better, I hope.
	!
	!	07/22/92 - Dan Perkins
	!		Don't pad job number with zeros.
	!		Cleaned program code.
	!
	!	09/09/92 - Dan Perkins
	!		Removed all references to BUYOFFLINE by putting
	!		BUYOFFLINE stuff in the BUYOFFLINE program.
	!		Cleaned code.
	!
	...
	!		Modified to blank out new field WP_BUYOFF::TOLOCATION
	!
	...
	!		Clean up source code so indenting makes much more
	!		sense.
	!
	...
	CASE OPT_INIT
		...
		SMG_WINDOW::NHELP = "WP_MAIN_BUYOFF"
...
20200	!******************************************************************
 ...
 Reenter:	SCOPE::SCOPE_EXIT = 0%
	...
	!
	!--
			WP_BUYOFF::JOB = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			WP_BUYOFF::ACCT = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, &
	...
	!
	!--
			WP_BUYOFF::OPERATOR = ENTR_3STRING(SCOPE, &
				SMG_WINDOW::WNUMBER, &
...
20350			!
 ...
 !			WP_BUYOFF::LOCATION = JC_JOB::LOCATION
		...
		CASE 2%
			WP_MAIN_BUYOFF = FUNC_TESTENTRY(SMG_WINDOW, &
...
32767	END FUNCTION
	...
	!	.b
	!	.lm +5
	!	Work In Process Buyoff Line Journal enters the number of
