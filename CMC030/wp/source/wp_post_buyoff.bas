1	%TITLE "Post Buy off Journal"
	%SBTTL "WP_POST_BUYOFF"
	%IDENT "V3.6a Calico"
	...
	!	.lm +5
	!	.b
	!	The ^*Post Buy off Journal\* process transfers quantity data at standard cost
	!	from the buy off journal to a specified Finished Goods inventory, depending upon
	!	whether a work order is a finished inventory item or a finished equipment ledger
	!	item.  The appropriate accounts are posted to the General Ledger system as well.
	!	.lm -5
	!
	! Index:
	!	.x Post>Buy off Journal
	!	.x Buy off Journal>Post
	!
	! Option:
	!
	! Compile:
	!
	!	$ BAS WP_SOURCE:WP_POST_BUYOFF/LINE
	!	$ LINK/EXE=WP_EXE: WP_POST_BUYOFF, FUNC_LIB:CMCLINK/OPTION
	...
	!		WP_BUYOFF::LOCATION sometimes.
	!		Use JC_JOB_EXAM instead of JC_JOB, which is never
	!		set up.
	...
	!		Fix calculation of REM.QTY to handle adjustments
	!		and other odd things better.
	!
	...
	!		Use MIN() instead of "xxx IF xxx < xxx"
	!
	!	11/19/2002 - Kevin Handy
	!		Remove a lot of commented out code
	!--
	%PAGE
	...
	!	to check the date of each transaction to ensure it falls within the
	!	specified dates before posting. A ^*Y\* entry causes the dates to be checked,
	!	while a ^*N\* entry causes all transactions to be posted.
	!	.lm -5
	!
	! Index:
	!	.x Check Dates
...
3100	WHEN ERROR IN
 ...
 LineItem:
	...
	GOTO CancelQty IF WP_BUYOFFLINE::COMPQTY = 0.0
	...
	PREVIOUS.BUILT = MIN(QTY(2%), TO.BUILD)
	FINAL.BUILT = MIN(QTY(2%) + WP_BUYOFFLINE::COMPQTY, TO.BUILD)
	...
	IC_TRANSACTION::COST		= &
		FUNC_ROUND(WP_BUYOFFLINE::COST * WP_BUYOFFLINE::COMPQTY, 2%)
 ...
 PostInv:
	REM.QTY = MIN(QTY(0%) - REM.QTY, WP_BUYOFFLINE::CANCELQTY)
...
5010	SMG_STATUS% = LIB$DELETE_FILE(WP_BUYOFF.DEV$ + &
		"WP_BUYOFF_" + BATCH_NO$ + ".JRL;*")

5020	SMG_STATUS% = LIB$DELETE_FILE(WP_BUYOFFLINE.DEV$ + &
...
32767	!******************************************************************
	! End of posting program WP_POST_BUYOFF
	!******************************************************************
	END
