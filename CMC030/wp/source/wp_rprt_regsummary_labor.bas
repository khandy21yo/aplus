1	%TITLE "Job Status Summary Report"
	%SBTTL "WP_RPRT_REGSUMMARY_LABOR"
	%IDENT "V3.6a Calico"
	...
	! Compile:
	!
	!	$ BAS WP_SOURCE:WP_RPRT_REGSUMMARY_LABOR/LINE
	!	$ LINK/EXE=WP_EXE: WP_RPRT_REGSUMMARY_LABOR, FUNC_LIB:CMCLINK/OPTION
 ...
 Init:	! Initilize report
	...
	!	The value in this field must be in agreement with field (01)
	!	Sort by.
	!	.b
	...
	!	this field must be in agreement with field (01) Sort by.
	!	.b
	...
	!	selects jobs with a given status code.
	!	.b
...
300	!
	...
	!
	! Bill Of Material Product Operation File
	!
	WHEN ERROR IN
		%INCLUDE "SOURCE:[BM.OPEN]BM_PRODOPER.OPN"
	USE
		FILENAME$ = "BM_PRODOPER.OPN"
		CONTINUE HelpError
	END WHEN
...
17020	!
 ...
 GetBom:
18000	!*******************************************************************
	! Get information from the bill of material file about hours
	! necessary to build at various levels
	!*******************************************************************
	...
	WHEN ERROR IN
		...
		BOMTOP = BM_PRODOPER::THISHOURS * QTY(1%)
		BOMLOW = BM_PRODOPER::HOURS * QTY(1%)
	USE
		!
		! Didn't find it, assume one per
		!
		BOMTOP = QTY(1%)
		BOMLOW = QTY(1%)
	END WHEN
